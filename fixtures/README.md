...
| `teimeris/` | Teimeris | 2 | `ayanamsha.json`: the engine's mean ayanamsha for every epoch-defined member at 24 Julian epochs from −700 to 2500, 1044 rows, written by the adapter's `ayanamsha-table` binary; `houses.json`: the engine's cusps and angles for twenty-one house systems at ten latitudes, two longitudes and three instants, 1260 rows, by its `houses-table` binary; `stars.json`: for the 128 members of the star table the engine knows by name, its catalogue record and its apparent, mean and geometric places at four instants (1900, J2000.0, 2023, 2100), with its obliquity and nutation at each, by its `stars-table` binary; `pheno.json`: the engine's phase angle, illuminated fraction, elongation, apparent diameter, magnitude and horizontal parallax for eleven bodies at sixteen instants with the geometry it read them from, and its equation of time with the Sun's apparent right ascension at each instant, by its `pheno-table` binary; `sidereal.json`: the engine's Greenwich apparent sidereal time under its default model and under its IERS 2010 model, with its Delta T, obliquity and nutation, at 41 instants strictly inside its 1850 to 2050 window and 8 at and beyond its bounds, by its `sidereal-table` binary (teimeris 0.1.0, 2026-09-05) |
