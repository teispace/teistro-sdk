...
**Last updated:** 2026-09-09, end of the sixtieth session (the daily
...
2. The local checkout is the repository root; `cargo xtask check-docs`
   ...
   2026-09-05).
3. **The next task is `check-parity` over the two new blobs.** The
   gate compares 103 values across the three bindings today, all from
   `positions`; a chart adds 72 distinct paths and a panchanga 85, of
   which 18 are the day they share — so 139 new values and 224 in all.
   Until it runs, "the three bindings agree" means something for
   `positions` alone, and the chart and almanac examples agree only
   because a person compared their output by eye.

   Both blobs are built: `ts_chart_found` founds a batch of charts and
   `ts_panchanga_days` a range of days, three bindings offer
   `found`/`foundMany` and `almanac`/`almanacDay` over them, and each
   ships a rectification and a panchangam example.
