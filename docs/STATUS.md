...
4. What is built, and what runs it. **Eight crates were missing from this
   ...
   | `tajika` | the annual chart: the Sun's return to where it stood at birth, in the chart's own zodiac, with the tropical and mean readings named as the rivals they are; the **Muntha**, the birth lagna progressed one sign per completed year, with its lord; and the five **office-bearers** of a year |
...
2b. **Tajika, after the office-bearers** (`03-design/muntha.md`, "The
   ...
   **the caller has founded**, and reproduce the source's worked year end
   ...
   `varsha_json.place` says. Left, in order: the Panchavargiya bala, whose
   Drekkana is one expression and whose Hudda table is the Egyptian terms
   with three transpositions (C109); and the Tajika aspects. Then the year lord
