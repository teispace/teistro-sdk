...
2. The local checkout is the repository root; `cargo xtask check-docs`
   ...
   2026-09-05).
3. **The next task is the chart document's round trip and its number
   grammar**, in that order, and then the JSON Schema they gate. The
   falsification pass for the schema is written
   (`cargo xtask schema` → `03-design/schema-measured.md`, gated by
   `check-schema`) and it falsified all five of its proposed rules:

   - the schema comes from **the API description**, not a sample and not
     a derive macro — a sample cannot tell a count from a whole double,
     nor give a string field its member list;
   - **nothing in the layer reads back**: 65 types derive `Serialize`,
     none derives `Deserialize`, so a stored document cannot be read
     into the SDK that wrote it and the schema's natural gate is half
     unwritable;
   - **the canonical form is not a fixed point** once a document carries
     an instant. The grammar's twelve decimals are three past what an
     `f64` resolves at a Julian day's magnitude, so a consumer that
     stores a document and hashes it again does not get the producer's
     hash. Fixing it moves the hash of every document, so it is a
     decision — `serial-and-the-envelope.md` §8 carries all three.
