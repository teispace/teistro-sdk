...
**Last updated:** 2026-09-22 (**a page measures what each binding's own
...
owns, and **29 of 173 members are named by nothing** — four in Node, three
in Dart, twenty-two in Python — listed rather than counted. Before it,
...
| date | what happened |
|---|---|
| 2026-09-22 | **A page measures what each binding's own surface has ever been used for.** The defect below was one instance of a class, so the class is now measured. `entry-point-is-reachable` holds that every boundary function is **placed** in a binding — exposed, declared, callable — and says nothing at all about whether anyone has ever called it. `cargo xtask exercised` asks the other question, of the layer a generator does *not* own: of the members each hand-written binding declares, how many does anything in that binding's own tests or examples name? **29 of 173 are named by nothing**, listed rather than counted — `dashaName`, `pack`, `range`, `unpack` in Node; `callJson`, `standing`, `twelve` in Dart; twenty-two in Python. A member counts as exercised when its name appears **after a dot** anywhere in the tests or examples, so a property read counts as much as a call and the count errs towards exercised: a member this page names is one nothing touches. Getting the three extractors honest took three passes and is worth recording — a Dart getter's body (`Foo get x => jsonDecode(y)`), a local helper declared inside a method (`Map<String, Object?> object(Object? v)`, four spaces in) and a multi-line call all read as declarations until the rule required a **member's own indent** and a **return type**. A measurement with noise in it is worse than none, because the list is the thing a reader acts on. |
