...
3. **Phase 3, the built-in ephemeris — milestone M3 — is all but
   ...
   **The first four of those are built.** The adapters export a plugin
   ...
   functions into what is callable today (**95**, 62 before plain structs
   were learned), what the adapter will never hand over whatever its
   shape (12, each of which opens, closes or rebinds the context every
   chart is cast on), and what is queued behind one more shape (54). Strings closed the way the page predicted they
