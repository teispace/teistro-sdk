...
3. **Phase 3, the built-in ephemeris — milestone M3 — is all but
   ...
   **The first four of those are built.** The adapters export a plugin
   ...
   functions into what is callable today (**135**: 62 before plain
   structs were learned, 95 before arrays, 112 before strings and pointers
   inside structs), what the adapter will never hand over whatever its
   shape (12, each of which opens, closes or rebinds the context every
   chart is cast on), and what is queued behind one more shape (14). Strings closed the way the page predicted they
   ...
   **Then strings and pointers inside structs, as one step: 112 → 135.**
   ...
   engine's own message** — and reading it exposed engine finding D3: the
   error record is not written by 255 failure paths, so after a null
   argument it still holds the previous failure's words; the passthrough
   attaches a message only when the record changed across the call. And
