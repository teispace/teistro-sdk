...
- Phase 1's remaining deliverables, in the roadmap's order: the `intl`
  ...
  the twelve-hour clock with the day periods, and transliteration with
  the derived `sa-Latn` locale (thirty-fifth session); still to come in
  it: the composite provider's precedence with the bindings, day-period
  ranges per locale and the `zone` option, XLIFF, rich renderers per
  binding.
