...
2. The local checkout is the repository root; `cargo xtask check-docs`
   ...
   2026-09-05).
3. **The next task is the JSON Schema emitter** for the chart document,
   beside the C header, the TypeScript surface, the Dart classes and the
   Python declarations. Everything it waited on is done: the
   falsification pass is written (`cargo xtask schema` →
   `03-design/schema-measured.md`, gated by `check-schema`), and of its
   six proposed rules the three that were about the bytes now **hold** —

   - the layer **reads back**: 60 of 65 types derive `Deserialize`, and
     the five that cannot are read by hand, by identity, checking what
     the document says about a shipped table against this build's;
   - **every number the form writes reads back** as the same double, the
     grammar having moved from a fixed twelve decimals to the shortest
     that round-trips;
   - **this build's parser reproduces a stored document's hash**, since
     `teistro-core` asks for `serde_json`'s `float_roundtrip`.

   The three still falsified are the ones that settle **where** the
   schema comes from: not a sample and not a derive macro, because a
   sample cannot tell a count from a whole double, cannot give a string
   field its member list, and does not know that one enum in a document
   is cased differently from the rest. The description knows all three.

   After it, the **Indian lunisolar calendar** `panchanga` still needs
