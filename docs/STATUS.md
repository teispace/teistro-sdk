...
**Last updated:** 2026-09-21 (the composers' first step: `crates/interpret`
turns a chart into a narrative plan of message keys and slots, which
`sdk.intl` renders — 93 recorded charts, 1878 items, all 3756 renderings
answering from the strict locale's own message with no fallback and no
warning, in English and Nepali, and `sdk.interpret()` carries it as a
ninth area. Next: the readings composer); before that 2026-09-20 (a rule renders to prose: `teistro_rules::prose`
