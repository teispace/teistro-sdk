...
**Last updated:** 2026-09-05, end of the fourth session (spike 2 done:
the binding toolchain decided as option A by measurement, ADR-0007; the
spike's code, generators and results under `spikes/02-binding-toolchain/`).
...
3. The next task is spike 3 below. Spike 1 is done: its fixtures are in
   ...
   holds the model for the Phase 1 extractor and generators.
...
- Phase 0 spike 3, ephemeris port: the port trait with the positions-only
  requirement, the Teimeris vtable adapter, a `sweph` host adapter under
  the containment rules of ADR-0019 (serialised gateway, no silent
  fallback, explicit flags, hashed data), frame completion for one body,
  the conformance kit prototype. The spike-2 slice's `EphemerisPort` and
  vtable are the starting point.

## Next

1. Spike 3 as above (it is "Now"); its result goes into the ephemeris
   port design page (`03-design/`, planned) and `02-architecture/`.
