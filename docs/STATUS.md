...
and Venus's house of joy read from the *Tajika Nilakanthi* itself — the
5th — where a widely used program reads the 12th, shipped as a named
rival. Before it, **a refusal names the value that failed**:
every JSON a caller writes — the chart request's records, the settings
patch, a rule request, a theme — names the path to the bad value,
`settings_json.frame.zodiacs` rather than `settings_json`, through one
reader in `teistro_core::strict`. Before it, **the sahams cross to all four
bindings**: `varsha_json.sahams`, `"all"` or saham keys, under
`sahamRules`, each year answering a ragged `year_sahams`; a saham is named
by the catalogue's kebab-case key every binding reads back, which the
first cut's snake case would have refused. Before it, **the Tajika sahams are built**, all
forty-one of the source's as formulas a caller can extend, and the
measurement moved their default: read off the chart's chalit, a house's
point was Vehlow's equal houses under two shipped profiles, so it is now
Sripati's built from the chart's own angles. Before it, **the sixteen Tajika yogas cross to all
four bindings** for the matters a caller names — `varsha_json.matters`,
`"all"` or a list of houses — under the `YogaRules` it passes; building
the crossing found that Node declared `noneAspects` while the wire read
`none_aspects`, so the rules now cross in one casing. Before it, **the annual chart crosses to all four
bindings**, and building the crossing found that `check-parity` reported
success while a runner crashed — *"2 bindings agree"* where there should
have been four, because a runner that was tried and failed was counted as
one that was skipped. It counts attempts now. Before it, **the annual
chart is built**: `crates/tajika`,
`sdk.chart().praveshas` for the instants and `sdk.chart().annual` for the
chart a year opens, with the three readings as named members and each
answer carrying the one that made it. The pass was moved onto the shipped
path and the page did not change by a byte. Before it, **Phase 7 began
with the annual chart, and the pass falsified the first shape of it by 18
arcseconds.** A solar
return is the crossing search the SDK already has; what it needed was the
chart's **own** sidereal reading rather than a frame's, because a frame
applies the mean ayanamsha and a chart the nutated one — seven minutes of
the Sun's time and two degrees of lagna. The shift now lives once, in
`teistro_astro::sidereal`, where the panchanga limbs and the annual chart
both read it. The tropical and mean-motion rivals are measured and priced
in lagna degrees: 177° and 5.6° at forty years. Before it, **a consumer
can register a dasha system of
either kernel now.** `RashiDefinition` closes the half that was missing:
sign-based rows a source states — `Sthira`, `Varnada` — were shut to a
consumer who **had** the text as firmly as to this build, which is a dead
end in the SDK rather than a gap in the sources. The kernel is stated in
the definition and never guessed from its fields, one registry holds both,
and a consumer's row answers period for period as the catalogued row it
copies in Rust, at the C boundary and in all three bindings. The coverage
gate measured its own repair, 3 and 19 becoming 5 and 17. Before it, **the
catalogue names forty dasha systems
and this build computes eighteen**, and which twenty-two were missing was
written nowhere. The gap is not a defect — a catalogue is a key space and
a build is a set of kernels — but an undeclared one is, so
`dasha-coverage-measured.md` counts both sets from the types, names every
system left with the blocker that would have to go first, and asks for
each of the 22 on a real founded chart to show the gap is a **refusal**
and not a dead end. Its queue is Phase 7's. Before it, **a count in prose
beside a growing thing, again**: the composers page said "nine of the
thirteen composers run over it", where the members are sixteen, ten run
and six do not — stale because the sixteenth landed the day before. It
is read off the calls now and names the six. Before them, **the audit was
widened from the 36
pages to all 99 files that say "do not edit", and the answer is that the
generated surface is held** — but only after two false alarms of the
audit's own making, both worth more than the result. The first sweep
detected failures by grepping `^FAIL`, and `check-catalogue` prints
`<path>: differs from its sources` with no such prefix, so the entire
generated catalogue read as ungated: **a detector that cannot see the
failure reports success**, which is the zsh loop again in another
costume. The second corrupted `teimeris.idl` — the **input** `cargo
xtask engine` reads — alongside the five façades it generates, so the
gate failed early on "does not parse" and never reached the comparison.
Corrupt outputs only. What survives is one real gap, already fixed:
`crates/ephemeris-builtin/src/tables/*.rs` are byte-ungated and
*behaviourally* gated, since a wrong coefficient moves `check-accuracy`
and `check-pluto` while a comment moves nothing, and the page declares
that byte-gate pending. Before it, **two generated pages were held by
...
`sections_for` — the one place that mapping lives — was held by nothing.
Alone and one at a time, because asking for several together hides a
missing section behind a sibling's: `strength` and `houses` each bring
the states four other composers read. Only `phala` is listed, for saying
what a **loaded** corpus carries where that context loads none, and the
list fails both ways — it caught my guess that `chalit` would be silent,
which is the **fourth** exception I guessed wrong today. What it does
*not* catch is written beside it, because a new gate is tempting to
believe covers the bug that prompted it: the almanac dead end had the
section asked for and the document's panchanga never turned into the
birth's limbs, which the every-key pass catches instead. And
`ASHTAKAVARGA`, the last row called a decision, **stayed one** when its
sources were read — there is no authority in the repository for which of
its eighty-four numbers a reading quotes, where the other three rows all
dissolved. Before it, **a settled question sat on the list of
...
consumer reads, along with `crates/interpret/README.md`'s *"the plan and
eight composers"* and the module catalogue's. All three are fixed the way
the lesson says: the count is **removed**, not updated, and points at
`PlanRequest::MEMBERS`, which is the authority and cannot go stale. A
fourth, in the ABI test, claimed one section serves *four* composers when
it now serves six. Before it, **two of the three remaining "decisions"
were not decisions, and the section queue is down to three rows**.
`BHAVA_BALA` was recorded as wanting *what a bhava must reach*: it wants
nothing of the kind, because a `BhavaStrength` carries the four parts and
their total and **no requirement at all**, so there is no verdict to
decline and no threshold to invent — and it needed **no new vocabulary**.
`VIMSHOPAKA` was recorded as wanting a knob, because saying four schemes
"would say the same graha four times"; it does, and so does every
per-graha composer — what would make that *repetition* is a message that
did not say **which** scheme it meant, and the fix was four words. Both
are built, the fourteenth and fifteenth composers. That is the **third**
time in one day a queue row's stated blocker did not survive its own
code, after `VAISESHIKAMSA` and `DASHA_PHALA`, which is worth more than
the composers: a reason written down without reading the source is a
guess with a citation. `ASHTAKAVARGA` is the one that stayed a decision —
which of eighty-four numbers deserves a sentence is a **reading**
question, not a mechanical one — and it was **decided** under the
delegation of 2026-09-22 and built as the sixteenth composer: the two
numbers a text quotes, each graha's bindus where it stands and each
sign's sarvashtakavarga, with no band, because the reading carries no
threshold and inventing one is what keeps `shadbala-strength` out of the
state corpus. The counts that used to stand here — `MEMBERS`, `KEYS`,
sections said — are on
[`03-design/interpret-measured.md`](03-design/interpret-measured.md),
which regenerates them; they had moved three times in this sentence. Before it, **a Hindi reader gets no item's frame in
Hindi, and that was a flag until today**. The locale roster says
`hi-Deva-IN`, `sa-Deva` and `sa-Latn` are at `base` completeness "until
their messages are translated", which tells a reader nothing about what
it costs. The founded chart measures it, on the page that
regenerates the figure rather than here: `en-Latn` and `ne-Deva-NP`
answer **every item** of its plan from their own messages, and the other
three answer **none** of them. It is not all-or-nothing, and the table
says that too — an item is a frame around **entity slots**, and those are
named in every shipped locale, so a Hindi reader gets the grahas, rashis
and nakshatras in Devanagari inside an English sentence. The work that
would close it is measured beside it: **69 messages over six
namespaces**, the largest being `sdk.calendar` at 28. Before it, **half
the composers' surface was
unmeasured, and a zero is a question**. The page that measures the
...
786 renderings, no fallback, nothing to warn about. The one key it cannot
reach is named with its reason, and the list fails **three** ways: a key
neither emitted nor excused, a key excused that the chart emits, and an
excuse naming something no composer emits at all. **Three of the four
excuses I first wrote were wrong**, and the check refused them one at a
time until only the burnt graha was left — which is the whole argument
for writing the check before trusting the list. Before it, **eleven
shipped rules could never hold,
and building a composer is what found them**. Q39 asked whether a plan
should say the almanac's own limbs; answering it *yes* meant asking the
document for the limb running at the chart's instant, and that turned up
a **dead end** of exactly the kind the maintainer's rule forbids.
`RuleInputs::of` built its chart with `panchanga: None` from every
document, and `with_rule_inputs` — which asks for exactly what the rules
read — never asked for the almanac, though `Rule::reads_panchanga` had
always been there to ask with. So the 11 rules that read a limb — six
arishtas, all four gandantas, one computed dosha — resolved to *"needs a
tithi, and the chart has none"* on every chart, honestly and
permanently, because nothing in the public surface could give them one.
`phala`'s four limb readings were unreachable for the same reason, which
is why the measured page has counted them at **zero** since they were
written and nobody asked why. Two lines were the fix, plus a helper that
reads the limb running at the instant out of the day's spans — and the
almanac gained `yoga_at` and `karana_at`, because two of its four moving
limbs could be asked for at an instant and two could not, though
`span::at` was already there for both. A test asserts all 11 are now
**answered rather than excused**. `panchanga` is the thirteenth composer;
`MEMBERS` is 13 and `KEYS` is 65. Before it, **verify caught a generated
type that
nothing declared, and the same shape as the Dart keyword before it**. The
key union of an entity kind was read from `sdk.entity`, which is the
right source for a member's **name** and the wrong one for its **key**: a
closed catalogue kind's members are a fact of the catalogue, and a locale
that names none of them does not make the kind smaller.
`sdk.phala.state` renders the `phala` form of `state.COMBUST` — a kind no
locale names, because the vetted tables have no word for being burnt —
and all three emitters wrote `state: StateKey` against a type nothing
declared. `check-intl` was green on it because the files were **up to
date**, and the Node and Dart type-checks that catch it run in the verify
matrix: forty minutes later, on five platforms at once, all five red. The
model seeds a closed kind's members from the catalogue now, and
`check-intl` reads the key types back out of what it just generated and
fails on a reference to an undeclared one — proved red on all three
targets by removing the fix. **A generated file being up to date says
nothing about its being right**, which is the second time that sentence
has been earned, so it is now a check and not a sentence. Before it,
**Q39 is open, and a lint now holds the
register to the tracker**. The section table raised it: `PANCHANGA` is
said only where a corpus carries a reading, so the almanac's own limbs —
and the **karana**, which no composer mentions in any form — are
computed, named in all five locales, and said by nothing. Everything a
composer would need is bought; whether a *plan* should recite them is a
design call, because this page has twice declined a message for being
the wrong kind of thing to say (a longitude alone is a fragment; a sign's
nature is a fact about the zodiac), and neither reason holds here. Three
options are recorded with a recommendation, and it is the maintainer's
because it is a third `PlanRequest` member in a day that has spent two.
**And the count of open questions is now refused in prose.** The resume
step's first sentence claimed an empty register with one question open,
then a single one with two — the same rot as the crate table and the
corpus size,
in the first thing a reader is told to trust. `open-question-is-named`
requires every question the register marks `open` to be **named** in the
tracker and refuses any phrase that counts them, both proved red. Before
it, **nothing cheap is left in the state
corpus: 214 readings reached no reader this morning and 47 do now**.
`planet-condition` closed last and shows the distinction worth keeping:
its seven readings key onto `dignity.EXALTED` and its neighbours and onto
`state.{RETROGRADE, COMBUST, VARGOTTAMA}`, which is exactly what
`conditions` already names — so that composer gained the vocabulary
parameter and says the reading **beside** the fact rather than instead of
it. `state`'s own members have **no name in any locale** and that did not
block it, because the message renders the `phala` form and never asks for
the name: an unnamed kind blocks a composer that would print its *name*,
not one that prints a form on its record. The 47 left are 39 in four
remedy families that want Phase 8's `remedies` to decide which graha or
rashi a remedy is *for*, 5 `special-lagna` waiting on a vetted name
table, and 3 `inauspicious-kaal` waiting on something that reads a
**time** rather than a chart. Before it, **the gate sweep I wrote down
was run in
the wrong shell, twice**. The rule in my own notes is "the simplest full
sweep is every `check-*` named in `fast-check.yml`, run in a **bash**
loop; zsh does not word-split `$(...)`" — and I ran it in zsh, where
`for g in $gates` iterates **once** with all 49 names as a single word.
It printed *0 failures* and meant nothing. Under `bash -c` the same loop
found two stale pages, and fast-check had already found one of them on
`326c990`. A sweep that cannot fail is worse than no sweep, because it is
believed; the loop is recorded again with `bash -c` in front of it.
Before it, **the native-review set was a flag in two
sentences and covered a tenth of itself**. The roadmap's Phase 6 exit
criterion asks for `ne` and `hi` sign-off, and which messages that covers
was recorded in two design-page phrases — *"flagged for the native
review"*, *"the Nepali of `sdk.reading` awaits it"* — written when two
namespaces existed. It is measured now: **69 Nepali messages say words of
their own**, across six namespaces, and the largest is `sdk.calendar` at
28, which no sentence had ever mentioned. The parser decides what counts,
so a message that only renders a catalogue value is left out — a reviewer
reading `{$graha :entity kind=graha}` is reading `sdk.entity` — and a
message rewritten from one into the other moves the list by itself. And
`hi-Deva-IN` carries **no `sdk.*` message at all**, which is worth saying
plainly: the roadmap asks for `hi` sign-off and there is not a sentence in
it to sign off, only names. Before it, **the twelfth composer, and the
section table asked for it**. `STATE` had read as *answered* because `conditions`
says every fact a `Placement` carries — nine of them, all said — while
the section's own `GrahaState` carries a dozen and the other half reached
no reader at all. `states` says it: how a graha stands to its dispositor
under all three friendships, and the four avasthas. **Almost every word
was already bought** — `Relationship` and the four avastha kinds are
catalogue members all five locales name — so six messages and not one new
term, with nothing for the native review it has not already seen, which
is the opposite of `dasha_phala` and why it followed rather than led. The
**undecided lajjitadi get an item of their own**, because `Lajjitadi`
names what holds, what is ruled out and what nothing in the chart
decides, and a plan printing only what held would turn *we cannot tell*
into *no*. `MEMBERS` is 12, `KEYS` is 56, and the state corpus's
shortfall is **54**, from 214 this morning. Before it, **137 state
readings reached a reader
today: 214 had no composer this morning and 77 do now**, closed in three
steps that each cost less than the last because each was chosen for what
it did *not* need. The timings needed no new subject (62). What the birth
nakshatra is needed no new computation (57). The dasha lords needed no
new name and no decision — only the **section** that makes the reading
this chart's rather than the zodiac's (18), which is why saying all nine
unconditionally would have been a table dump and the same objection still
holds `graha-color` back. Before it, **the eleventh composer, and the
first built off the queue that found it**. `dasha_phala` says what each graha's
placement says of its dasha — when in the dasha its effects come, whether
its place is auspicious, the points its dignity earns and whether the
placement tilts the dasha (BPHS ch. 28 vv. 7 to 10, ch. 47 vv. 3 to 6) —
with the reading a loaded corpus carries of that graha as a dasha lord.
The section table named it the cheapest of the six unsaid sections, and
it was still words: its `nature` is a catalogue member **no strict locale
names**, so it is said the way `sdk.reading.lifeClass` says a class of
life — matched on the key, with the words written in each locale, rather
than through an entity slot that would print nothing. The tilt is one
item and not two, because vv. 5 and 6 let a placement be both favourable
and unfavourable and the message has a third arm for it; a placement that
tilts neither way says nothing of the tilt. `dashaPhala` crosses the C
boundary and is declared and *exercised* in all three bindings, each
rendering every item it says with no fallback and no warning.
`PlanRequest::MEMBERS` is 11 and `KEYS` is 46. Before it, **six of the
eleven sections a chart
document can carry have no composer at all, and the page that said
otherwise now enumerates them**. `interpret-measured.md` carried the
sentence *"the silences that remain are all of that kind"* — meaning a
computed value with no catalogue member — and it was true of the
...
it, **57 more readings reach a reader, and the
reason they were cheap is the test for the ones left**. Every nakshatra in
the catalogue already carries its **gana, nadi, yoni, varna and element**
as attributes, and the corpus keys its readings onto those kinds' own
records — `gana.DEVA`, `yoni.ELEPHANT`, `tatwa.PRITHVI`. So the subject
of each is *settled* by the nakshatra `phala`'s panchanga limb already
names, which is the Moon's and therefore the janma nakshatra every text
reads them from. `namakarana` is a second form on the nakshatra itself,
exactly as `lagnaPhala` is on a rashi. **Nothing computed, nothing
chosen, no catalogue kind decided** — and that is the test for what
remains: a category whose subject the SDK already computes *and already
names* can be said without a decision. `graha-color`,
`graha-direction`, `mantra-ritual` and `ishta-devata` fail it not for
want of a kind but because *which* graha or rashi a remedy is for is a
rule this corpus does not record, which is Phase 8's `remedies`; saying
all nine would be a table dump rather than a reading. 95 of the 425 are
left, from 214 this morning. Before it, **62 readings reach a reader that
nothing said before**, which is the largest single step off the corpus's own
shortfall: 214 of the state readings had no composer that said them and
152 do now. `dosha-timing` was the one that could close without a
decision, and the reason is worth keeping — it keys onto **rules the
`readings` composer already walks**, so nothing new had to be computed
and no catalogue kind had to be permanent. A rule's reading and a rule's
timing are two forms on one `rule.<KEY>` record, `sdk.reading.timing`
renders the second exactly as `sdk.reading.says` renders the first, and
the composer asks for them **independently**, because 34 rules carry both
and 18 carry a timing and **no reading at all** — every computed dosha,
whose verse states no effect and which therefore said nothing in words
until now. The message has no prose of its own in any locale, being the
entity render and nothing else, so it adds nothing to the `ne`/`hi`
native review. 124 new renderings, no fallback and no warning. The table
that says which message says which reading is hand-kept and now guarded:
every message it names must be in `teistro_interpret::KEYS`, or the page
would count a reading as *said* while rendering a key no locale carries.
Before it, **a lint holds this document's own crate
table to `crates/`**, because it had gone stale by eight entries — `rules`,
`interpret`, `dasha`, `strength`, `sdk`, `geometry`, `render-svg` and
`ephemeris-builtin`, which is most of what Phases 4 to 6 produced — and
nothing noticed, since nothing compared it to the directory. It is the
third stale claim found in this file in one day, after the gate list and
the submodule pin, and the first one that is a *list beside a thing that
grows*, which is the shape a lint can hold. `crate-is-listed` reads
`crates/` and the table's first cells and fails **both** ways: a crate the
table never names, and a name the table carries that is not a crate. Only
the first cell, because the `chart` row's description names `day`, `bhava`,
`zodiac` and `foundation`, which are modules — a looser read reports four
crates that do not exist. It was proved red by renaming one row before it
was believed. Before it, **the plan itself is corrected where this
...
4. What is built, and what runs it. **Eight crates were missing from this
   ...
   | `tajika` | the annual chart: the Sun's return to where it stood at birth, in the chart's own zodiac, with the tropical and mean readings named as the rivals they are; the **Muntha**, the birth lagna progressed one sign per completed year, with its lord; the five **office-bearers** of a year; the **Panchavargiya bala**, exact in sub-sub units; the **lord of the year** chosen among them; and the **Tajika aspects** with their deeptamsha orbs, Ithasala and Ishrafa |
   | `gochar` | the transits read from the natal Moon (Phaladeepika ch. 26): each graha's good houses, its vedha house and **which grahas obstruct it** after the verses' mutual exemptions, the verdict, and the decanate where a transit bears fruit; the nodes' vedha and whether they obstruct are settings |
...
Phases 1 and 2 have both met their exit criteria. **Phases 5, 6 and 7 are
open and running beside one another** — strength and dashas since
2026-09-15, rules and interpretation since 2026-09-15, and techniques
since 2026-09-22 with the annual chart and the Muntha — each started, as
every module since Phase 4 has been, with its measurement. Which exit
criteria remain is each phase's own section of
[`07-roadmap/00-roadmap.md`](07-roadmap/00-roadmap.md), which is the
authority and is not restated here, because a list of what remains
written in two places is two lists that drift. What each closed phase
left behind is listed where it was deferred, and the session log below
is the record of how each was built.

The account of Phases 3 and 4 that follows was written while they were
the open ones, and is kept as that record.
...
| date | what happened |
|---|---|
| 2026-09-26 | **Gochar begins: the Phaladeepika table, read on the page.** Phase 7's next module. The research page asks for the vedha table "with citations; confirm the exemptions"; Phaladeepika ch. 26 gives it, and its OCR reads one numeral as "Bill", so vv. 1 to 8 were read in the Sanskrit on the printed pages (286 to 288). That settled two of three suspected forks: v. 8's "bad effects if marred" is the same good-unless-obstructed rule as the others, and Mercury's pairs are the translation's. Nothing to measure against — the corpus records no transit and PyJHora has no gochar or vedha — so the table is asserted cell by cell, v. 2's good houses written out separately in the test so the vedha table's first column is checked against the verse and not itself. Built: `teistro-gochar` (core only), a `gochar` settings group for the two silences (the nodes' vedha, whether the nodes obstruct), and `sdk.chart().gochar` over a batch of instants, its reference the natal Moon (v. 1) or, asked, the lagna. Adding the group found `SettingsPatch::is_empty` had left `panchanga` out, so a panchanga-only patch called itself empty; it now compares with the empty patch, and a test sets each knob of every group alone. Next: the measured pass, the boundary, then Sade Sati once a source for its phases is in hand. |
| 2026-09-26 | **Q40 decided by measuring its premise; the nodes spared each other.** Handed the question to research and settle. A and C both assumed the engine's own sunrise and zodiac are the Swiss-exact ones; measured against the Swiss-based recording they are not: the SDK's solver 9.77 s at worst over 55 charts against Teimeris's own 32.39 s, and the port's ayanamsha is the mean one, 18.46″ from the nutated value the recording applies and the catalogue reproduces to 0.0086″. So B: a modern chart keeps the SDK's day and zodiac, ADR-0013 amended, and the adapter's fixture test asserts the order and names Q40 if it flips. Found on the way: the completion took a modern provider's mean ayanamsha on either basis; it now answers the mean alone, the catalogue the true one, `native-only` refusing (proved red). Earlier the same day, planning the gochar pass found the literal reading left the nodes never good (always opposite, the Sun's vedha pairs opposite houses); `node_obstruction` gained `NOT_EACH_OTHER` as the default before the knob shipped (C140). |
| 2026-09-26 | **C135 closed by research rather than a knob.** Planned as a new knob for the shipped padas' two-lorded signs, the research found v. 7's द्विनाथ arises only where a node co-lords Scorpio or Aquarius: under the default `jaimini.node_co_lordship = NONE` every sign has one lord, the catalogue's, so the padas were already the verse's and the corpus's at once. So the padas follow the existing knob — the stronger lord by ch. 46's ladder under `STRONGER_LORD` or `BOTH` — and nothing moves under the default. Built without widening a hot type: `points::arudha` gains `_by` forms, `dasha::jaimini::pada_lord` names the lord, the façade's one `placed_chart` feeds the arudha lagna of the sign dashas and the Vaiseshikamsa, and the rules take the twelve lords through `Evaluator::with_pada_lords`, beside `with_points`, leaving `RuleChart`'s and `Readings`' ~75 literal sites untouched. Two defects found on the way: the façade never re-exported `teistro::points`, so a Rust consumer could not reach the padas at all; and the arudhas pass counted with its own private copy, so a change to the shipped count could not move its page — it now calls `arudha_by`, every existing row unchanged, and measures the verse's reading at 46 of 852. A swallowed-error fallback I first wrote into the rules' lord lookup was taken out before commit. |
| 2026-09-26 | **The grahas' arudhas, and a verse that reaches the shipped padas.** BPHS ch. 29 vv. 6 and 7, read on the printed page because the volume's OCR is Latin noise: the count is stated (as many signs on as the own sign stands from the graha) and v. 7's सबलावधि takes the stronger of two lords or two signs; the nodes and the bhava arudhas' exception are the translator's, not the verse's. PyJHora, measured over 600 charts, applies the exception from the graha's sign on 2 400 of 2 400 and chooses a planet's stronger sign by no reading of ch. 46 past 82%. Built: `graha_arudha`, the nodes through the existing `jaimini.node_co_lordship` rather than a second knob (C133), the ladder C51 settled for the two-sign planets (C134), and a knob `jaimini.graha_arudha_exception` for the move (C132); the reading gains `graha_arudhas` and the per-graha section is renamed `jaimini_grahas` before any release carried its old name. A test's expected sign was wrong and the code right — the move is to the 10th from where the count landed. **Reading v. 7 found a gap in shipped code**: `points::arudha` counts Scorpio's and Aquarius's houses to one lord where the verse says the stronger; the padas were derived from the recording engine (0 of 852 disagree), so this is registered as C135 and left for its own change rather than moved silently. |
| 2026-09-26 | **Jaimini's significators reach every binding.** The document gains a `jaimini` section (`ChartRequest::with_jaimini`, and in `with_everything`), the C boundary `TS_CHART_JAIMINI` (bit 2048) and sections 51 and 52, and Node, Python and Dart `chart.jaimini`, each mirroring the Rust type, with parity across all four. Three layout decisions, each held by a test: `qualified` crosses as a nine-graha bit set, which is lossless only because every rule lists it in catalogue order once each — a test holds that over every lagna, rule and co-lordship rather than trusting it; the Brahma graha's presence and the reason for its absence cross as **one** code, `TsBrahmaOutcome`, so they cannot disagree, and the serde-spelling gate checks its three reasons against `NoBrahma` with `FOUND` named as the one key no Rust type spells; `TsBrahmaRule` crosses a `non_exhaustive` knob through a wildcard, so a test requires every member to cross distinctly. `sdk.chart().jaimini` now answers a stored document with its own section and computes the states once. `graha_bits` became a view of a sixteen-bit `graha_mask`, and `charts()`'s three text sections a helper, the IDL diff proving both additive. The three earlier Jaimini PRs had added no changelog entry; this one covers all four. |
| 2026-09-26 | **Sudasa was filed under the wrong blocker.** The coverage page said it waits on the karakamsha, and the karakamsha is now built — but no source read starts Sudasa there. BPHS does not name it among its dasas (ch. 46 vv. 6 to 11); two Sanskrit editions of the Jaimini sutras state no Sudasa or Sree lagna in plain words; PyJHora's signature takes the Sree lagna's house and longitude. So its hardest blocker is the text, and the page says so; a Sree-lagna start and a fractional first period follow once a verse is in hand. Built on a guess it would have been a table invented rather than cited. |
| 2026-09-26 | **C51 reopened by a worked example and settled by the verses it illustrates.** The Yogardha's printed table, and the Chara table it builds on, count Scorpio to Mars, alone in fixed Taurus, where C51's ladder takes Ketu, alone in dual Sagittarius. Before measuring readings against the corpus, the Sanskrit of vv. 158 to 166 was read on the printed page (p. 554): v. 161 a sign with a graha over one without; v. 162 बली तत्राधिकग्रहः, the one with more, and with the grahas equal राशिबलाद् बलम्, the sign's own strength; v. 163 movable, fixed and dual in rising strength, and **only when that is equal** the greater count. That is the ladder as built, so the verses take Ketu and the example contradicts them; C51's default stands and the test's comment says why. The third time today the Sanskrit and its translation's apparatus have parted (C129, C131, and this), and the second time reading the verse cost less than the measurement it replaced. |
| 2026-09-26 | **The Yogardha dasa, and a crux reopened by its worked example.** v. 174, read on the printed page, states the span (half the sum of the Chara and Sthira years) and the start (the stronger of the lagna and the 7th) and **no order**; the design page had said it did, which was the translation's. The translation's order, by the start's parity, is also its worked example's and PyJHora's on 200 of 200, so it is the default (C131). It is **not** a composition over kernels, as the coverage page had it: it takes Sthira's years and not its start, needs no Brahma, and is one `Length::MeanOfCountAndModality` in the sign-based kernel, whose `RashiRow::years` now answers `f64` half-years; its start reuses Mandooka's `stronger_of: [1, 7]` under `dasha.rashi_start`. The acceptance test reproduces the printed table after v. 174 but two cells, each named and held both ways: Leo, the book's slip against its own Chara table (7½ is computed), and Scorpio, where both of the book's tables count to Mars against C51's modality step — so **C51 is reopened**, to be measured against the corpus before its default moves. The page image also confirms v. 168 reads चाऽष्टौ द्वन्द्वे नव. PyJHora gives every mahadasha the same antardashas on 720 of 720, which no text states; the kernel's stand. This build computes 24 of the catalogue's 40. |
| 2026-09-26 | **The Sthira dasa, from the Brahma graha and nowhere else.** Built on the significators: the sign-based kernel gains `Start::Brahma` and `Order::Forward`, and `RashiRow::start_sign` became fallible so a Sthira dasa over a chart whose rule finds no Brahma is **refused** on `jaimini.brahma`, naming the translator's note as the rule that supplies one, rather than started from the lagna the coverage page had stated. `RashiChart` carries Brahma's sign, filled by the façade under the settings. The build computes 23 of the catalogue's 40; the coverage page's Sthira row and its registrable entry are gone; the catalogue cites BPHS ch. 46 vv. 168 to 173. The corpus does not record Sthira, so the dasha tests list it as the one shipped row unrecorded, held both ways, and the façade test holds both a found and a refused chart. Its antardashas run forward from their own sign as its periods do, where every other sign-based row runs them by parity; PyJHora's run so on 480 of 480, 240 from an even sign (C129). **Building it found `with_everything` refused on nearly half of charts**, since it asked for every computed system and a Sthira with no Brahma refuses the reading; it now asks for `dasha::systems_every_chart_gives()`, derived from `Start::every_chart_gives`, and the Sthira dasa is asked for by name. |
| 2026-09-26 | **Jaimini's significators: the karakamsha and the Brahma graha, and the Sthira dasa's row was wrong.** The coverage page listed `STHIRA` as "from the lagna, consecutive", unverified. BPHS ch. 46 v. 169 in the Sanskrit starts it from **the sign the Brahma graha occupies**, and vv. 170 to 173 find Brahma: the lords of the 6th, 8th and 12th from the stronger of the lagna and the 7th, in an odd sign behind it, the most degrees; and where Saturn or the node qualifies, the planet in the 6th from it — which the translation reads the other way. PyJHora, run as a black box over 600 drawn charts, reproduces the worked example (Mercury) and fits no reading of the verses past 50%, and runs the Sthira dasa forward from every sign, as the Sanskrit's silence on order reads; the translation's "reverse from even signs" is its own. **The verses find no Brahma on 45% of charts** and give no fallback, so the default refuses by name and the translator's note is a knob (`jaimini.brahma`). Seven forks registered (C124 to C130), `dasha::jaimini` built with the worked example as its test, `sdk.chart().jaimini` reading both significators, and two knobs read for the first time (`jaimini.chara_karakas`, `jaimini.node_co_lordship`). Measured over the corpus (`jaimini-measured.md`): the Atmakaraka agrees with the recording on 54 of 55, the miss a Sun 0.02″ from Aries on either side in the two ephemerides; the karakamsha's houses differ between the rasi chart and the navamsha on 446 of 495. Next: the Sthira dasa from Brahma's sign, and the reading at the boundary. |
| 2026-09-26 | **A profile for the text, and the two dead ends researching it found.** A `surya-siddhanta` profile looked like one patch; reading the settings first found the `frame.siddhanta` knob had been read by nothing since it was added (its lint exemption waiting for "a caller asks the SDK to pick a provider"), and that **every settings warning was said to nobody**: `Resolved.warnings` were computed at resolution, documented "for provenance", and carried into no result. ADR-0029 decided the knob's reader: the settings **ask** and the chain **supplies**, so the knob never opens a provider; it is held to the one opened, a modern engine under the Surya Siddhanta refused at the context and the text's provider under DRIK settings warned (`classical-provider-drik-settings`). `Resolved::provenance` is now the one constructor four producers stamp from, and it carries every warning into every result's provenance in every binding. The profile is `parashari-classical` with the text's astronomy and ayanamsha. The boundary's documented profile list was prose every binding copied; a test now holds it to `SHIPPED_PROFILES` both ways. |
| 2026-09-26 | **The Surya Siddhanta's chart, the text's in every part.** The measured page's last three hybrid parts are the text's now. **The day**: `DrikSun` asks the provider's own horizon search where a classical astronomy defines the sunrise (`Completion::defines`), falling back and stopping claiming the day where the text refuses a convention; and building it found both solar models' midnights rounded to the second through `LocalMeanTime`'s whole-second offset, half a second from the text's own — `solar::local_mean_midnight` is now exact, and no calendar page moved. **The angles**: a port override, `Overrides::ANGLES`, answered by the text's Lagna and meridian (III.46 to 49); the nine house systems built from two angles alone (`houses::FROM_ANGLES`, refactored onto one `AngularFrame`) and the thirteen that need the sphere refused by name. The design page's vtable slot was dropped once building it asked who would fill one. **The report**: `zodiac`, `angles` and `day` steps, the envelope's never-filled `deviation`, and `sdk.chart().angles`, which a test caught answering the sphere's angles for a classical chart over another provider before it refused. `Ephemeris::SuryaSiddhanta` opens it by name in Rust, and `TS_EPHEMERIS_SURYA_SIDDHANTA` in every binding, whose suites and the parity gate now hold a classical chart. |
| 2026-09-26 | **A chart founded on the Surya Siddhanta, measured through the SDK, and two parts of it put right.** STATUS Next 1 left the classical provider's Lagna and hours for "the chart layer when it exists". It exists, so I founded all 55 recorded births through the façade over `SiddhantaProvider::text()` and held each part against the text (`cargo xtask classical-chart`, held by `check-classical-chart`). Every part was a hybrid: the zodiac the catalogue's `SURYASIDDHANTA` member, 1.6° from the text's own precession while the stamp said `ayanamsha:NATIVE`; the Lagna the SDK's spherical ascendant, up to 8° off and in another sign on 6 of 53; the day the SDK's solver over the text's Sun, up to 24 minutes from the text's sunrise and another hora on 6 of 53. The text's 24° obliquity closes none of the Lagna's gap: the text's method does (III.46 to 49). The principle (`03-design/classical-chart.md`): a classical provider's overrides are **definitions**, so under `prefer-native` the chart takes them. Built: the zodiac from the provider where a classical astronomy defines the member (`Completion::defines_ayanamsha`, `ChartZodiac::defined`), and — found by building it — the corrections step no longer moves a classical text's place to an apparent one, which had read the text's mean distances as astronomical units and given its Moon an 8.3-minute light time (0.078°); it now refuses any distances not in astronomical units. The nine grahas are the text's to the last bit. Left: the day, the angles, the stamp, and reaching it by name. |
| 2026-09-26 | **The air at the horizon (C34), measured before it was built.** The fixed 34′ ignores height; Teimeris's own sunrise moves 26 s later at Kathmandu and 56 s at La Paz as its standard air thins. **Researched**: the models agree at the reference air (34.46′ to 34.48′) and part with the temperature; Sinclair's correction, which Teimeris applies, has no citable source, so the SDK refracts by Bennett scaled by the air (Meeus ch. 16, as Skyfield does). **Built**: `day.sunrise` gains `ATMOSPHERIC { which, air }`, a variant so every settings hash stands, each part of the air defaulting to the engines' standard at the observer's height (ICAO pressure within 0.15 hPa of Teimeris's to 8,848 m, 15 °C); `Refraction::Atmosphere` crosses the vtable resolved (ABI 4), so an engine is asked about the same air; the day section and every binding's day report the air applied. **Measured**: at the 20 charts at or above 1,000 m the SDK and Teimeris part by 3.7 s in the median and 5.8 s at worst where the fixed 34′ parted by 29.9 s and 61.8 s. |
| 2026-09-26 | **The wasm package finished, and a Worker it could not have run in.** `@teistro/sdk-wasm` is released with the others and checked five ways (the Node suite unchanged, headless Chrome, installed, Cloudflare's workerd, and the parity gate's fifth runner), 4.77 MB and 1.33 MB gzipped under a budget held both ways. Measuring the size found `compact` had never been buildable (ADR-0028 amended). `wasm-opt` was measured and declined: 7% smaller raw, 6% **larger** gzipped, which is what ships. **Researched before building**: a Worker cannot compile wasm at run time, and without a `workerd` condition the package bundled silently and died at start (`Invalid URL string`); a precompiled-import loader and a Wrangler-bundled `workerd test` step close it, the bundle 1.37 MB gzipped. Wrangler needs Node 22, and Node 20 had left support, so the floor moved before the first release, and a new lint holds `engines` and every workflow to it — it found the adapter already stating a different floor (`>=20.11`). |
| 2026-09-26 | **Gochar measured, and its batch stops founding charts it does not read.** Pricing the batch first: `sdk.chart().gochar` founded a whole chart per instant (day, houses, two lagnas) to read nine longitudes, and assembling the sections was measured separately at nothing visible — the founding was the cost. `Founder::longitudes` places the grahas at every instant in one request and skips the speeds' second request, each place equal to the founded chart's to the bit (held under the default, the topocentric conformance profile and the true node, proved red by a 1e-9° nudge): 2.4× (110 ms to 46 ms for 240 instants). The pass (`gochar-measured.md`, `check-gochar`): over every day 1960 to 2020 from every reference sign, each of the 72 ordered pairs obstructs always, or is spared by exactly the verses' exemptions and C140, or never stands there (Mercury and Venus never oppose the Sun) — three lists written in the pass, held both ways, an unused bound an error, proved red by dropping one exemption; the literal node reading measured (175 320 of 175 320 obstructed by the other node); over the births the lagna moves 53% of verdicts (C139). Next: the boundary and the bindings. |
...
| 2026-09-24 | **The year lord's Moon, from both books, and a default that was wrong.** Charak's chain passes the Moon over for the next claimant that aspects. Where nobody is next, his text says it "still does not become the year lord": the planet in Ithasala with it takes the year (the strongest of several), and otherwise the lord of its sign. **The build answered the Moon itself there**, because the step-down fell through. §18 of the measured page counts **26 of 2 159 years** it had given to the Moon. Read against the *Tajika Nilakanthi*'s Varshatantra vv. 9–12, the author's own view (v. 12) is Charak's "some authorities": the successor at once, with no step down. Its commentary says a Moon in its own Cancer is its own successor, which settles the one case the rule seemed to leave open. The corpus has one such year. The build now succeeds the Moon wherever the chain would land on it. `MoonMayRule::Ithasala`, `MoonPartner::OfficeBearer` (one commentary narrows the successor to the office-bearers) and `NoneAspects::Strongest` (v. 11) are knobs. `VarsheshaRules::drishti` reads the Ithasala as the yogas do, and `Chosen` gains three steps and `Chosen::ALL`. **Chart VII-1 was read off the page** instead of transposed: Gemini rising, where the old fixture had Aries. It reproduces the book's Moon (12:28:15) and Mercury (10:49:15) to the sub-unit, Venus as year lord by the step-down, and Mercury, the Moon's sign lord, by the Nilakanthi's reading, both as printed. The book's Venus, 9:06:30, is not reproduced: the chain gives 7:14:00, one house relation read as neutral where the book's own positional rule makes it an enemy. It is recorded and not fitted. Also: **`MoonMayRule`, `Tied` and `NoneAspects` had never been exported**, so a Rust caller could set them only through serde, and the rust-surface gate misses a field's type. Dart's `VarsheshaRules` took raw strings with the defaults repeated; it is typed now, with null meaning the SDK's default. The parity runner's four hand-written key tables became `wire_key`. Still open under C106: the Nilakanthi's tie broken by the greater aspect, and a benefic's Ishrafa with the Moon. Next: the four annual dashas. |
| 2026-09-24 | **Kuttha, and with it all sixteen.** C117 was the only thing standing between Kuttha and shipping: Charak names Tajika's malefics and no benefics. The *Tajika Nilakanthi*'s Kuttha verses (Ṣoḍaśayoga vv. 67–68), which Charak condenses, settled it. Gangadhara Mishra's commentary glosses "by a benefic" as any one of the full Moon, Mercury, Jupiter and Venus, and the verse's malefic reaches by the *kṣuta* aspect (1, 4, 7, 10) or a shared sign, which is `malefic_reaches` word for word, so Kuttha shares Manau's and Rudda's reading instead of adding one. Charak's saham chapter names the same four, so `BENEFICS` is one constant and `SahamNatures::Chapter` now reads it. Each lord's four clauses are a `Favour`, reported by `favour` and `sdk.chart().favour` as `affliction` is for Rudda, and `YogaRules::moon_benefic` carries the commentary's *full* Moon as `waxing`, across every binding. The verse's other marks of strength (risen, direct, slow) are alternatives, so they are not added as conditions. Measured (`muntha-measured.md` §17): Kuttha holds in **958** of 22 009 judged matters, of 9 042 strong pairs. The malefic clause turns away the most, 2 708 of them alone. The waxing Moon keeps 897. The pass fails if `favour` and the yoga disagree anywhere, if the waxing reading ever adds a Kuttha, or if Kuttha passes the new strong-pair ceiling. **`fill` had been dropping the final newline** of any generated page whose last block is prose. Fourteen pages ended without one, and the generator now keeps it. Next: the year lord's two Moon readings, and the four annual dashas. |
...
| 2026-09-23 | **The sahams cross.** `varsha_json.sahams` is `"all"` or saham keys in the caller's order, and `varsha_json.sahamRules` carries `{addSign, houses, roga}`; each year's `annual_charts` row counts them in `saham_count` and `year_sahams` carries each saham, its longitude, sign, lord, house and whether a sign was added, decoded in Node, Python and Dart and held equal by four parity runners under the source's rules and under every rival at once. **The spelling was the finding.** The generated catalogue keys every binding reads back are kebab-case (`karya-siddhi`) and serde wrote the wire in snake case, so a Node caller writing the declared `Saham` type would have been refused — the `noneAspects` trap in another shape; `Saham` now serialises as its catalogue key and the ABI test holds the two equal member for member. `matters` and `sahams` share one reader, `Asked<T>`, and the ABI tests' three copies of one chart request became one helper. A value error inside a rule record still names only the record (`varsha_json.sahamRules`), as `yogas` does; naming the path to the value is SDK-wide and its own unit. Next: that, then the Harsha bala. |
| 2026-09-23 | **The sahams are built, and the measurement moved their default.** Charak's ch. XI gives **forty-one**, not the thirty-six the research page carried from JHora and PyJHora — and PyJHora's thirty-six are not a subset (it adds Apamrityu and lacks six). Every saham is one shape, **a − b + c**, carried a sign on when c is not between b and a, so a saham is **data**: a `Formula` over a closed set of `Term`s, the forty-one a table of them, and a caller's own saham — Apamrityu, or another authority's Asha — goes through the same memoised evaluator. **The source settles two readings and a black box a third.** All seven sahams the source works come back to the printed arcminute, four of them end to end from a birth and a year the SDK founded, and its birth-chart Punya — Sun, lagna and Moon all in Leo — refutes the rule PyJHora applies, which I measured over 600 probe charts (values only) as counting "between" in **whole signs**, fitting all 600; it ships as `AddSign::Signs`, and a test reproduces sixteen of that program's sahams by day and night to 1e-9 under it. **A zero was the finding.** The first build read a house's point off the chart's chalit, and the measured page said equal houses moved no house saham in any chart: the conformance profile's chalit is **Vehlow's**, equal houses centred on the lagna, and so is `nepali-default`'s — so under two shipped profiles the source's reading silently was not the default. The default is now **Sripati's mid-points built from the chart's lagna and midheaven**, the source's own recipe, reproducing all twelve of its printed mid-points; `teistro_chart::foundation::angles_of` gives a founded chart its midheaven with no ephemeris, and a test changing only the chalit holds the sahams equal. Read correctly, equal houses move the house sahams in about a fifth of placements. The pass fails on two identities, and the first version of one was wrong — a saham reading another moves with it — which the pass caught on its first run. Next: the sahams across the boundary. |
| 2026-09-23 | **The sixteen yogas cross, and the question of which matters is the caller's.** `varsha_json.matters` is `"all"` or a list of houses and needs `place`, because a matter is read from the annual lagna; `varsha_json.yogas` carries the `YogaRules`. Each year's chart answers a ragged `year_matters` section — house, sign, lagnesha, karyesha, `same_lord`, the pair's own relation and `unanswered` as a bit set over `TsYearYoga` — with `matter_yogas` beneath it and `matter_legs` beneath those, three deep. **Batch is in the signature**: `tajika_yogas_many` judges the rules, the states and the seven strengths once for a chart and answers each matter from them, and a test holds it equal to asking each alone. `Held.between` was only ever the matter's own pair or nothing, so it crosses as a flag, and the encoder **refuses** a row that breaks that rather than writing a column nobody reads. **A defect in the last crossing surfaced**: Node's `.d.ts` promised `noneAspects` and the wire read `none_aspects`, so a Node caller following its own types was refused. Both rule sets now read camelCase, Python and Dart translate, and each binding tests the key it promises. `why` does not cross; `unanswered` does, so a consumer can still tell *did not hold* from *cannot be told*. 4 bindings agree on 10 466 values, and the three examples print alike. Next: the 36 sahamas. |
| 2026-09-23 | **The Tajika yogas answer a question, and building them proved the design page wrong twice — which is what the page is for.** `sdk.chart().tajika_yogas(&annual, house)` gives the four of the sixteen that need only the pair's own aspects: Ithasala in all three kinds, Ishrafa, **Nakta** and **Yamaya**. Fourteen of the sixteen are judgements about the **lagnesha** and the **karyesha** — the lord of the annual lagna and the lord of the house the matter belongs to — so the same year answers differently for each of the twelve houses, and *what yogas does this year have* is not a question these sixteen answer. **Nakta and Yamaya wanted a rule the aspects did not have**: the source measures a third planet's reach by **its own** deeptamsha, not the mean it would share with each of the pair, so the orb came out of `between` into a crate-private `between_within` rather than the four bands being written a second time. The two are one pass, not two: they ask the same thing of every other planet and differ only in which way its speed falls — faster than both carries the light (one leg Ishrafa, one Ithasala), slower than both gathers it (both legs Ithasala). Over the corpus: Ithasala 4 250, Ishrafa 3 050, Nakta 637, Yamaya 413, of 25 908 matters. **The design said `Bhava::Seventh` and there is no `Bhava`, nor should there be.** A house is named by its number, and a `House` newtype already existed in `teistro_rules::language` with three other crates reaching for it. It moved to `teistro_core` rather than being copied — a second twelve-valued primitive is a second place to get the counting wrong — and gained `sign_from`, the inverse of its `between`, held by a test over all 144 pairs. Core's own error replaced its `String`, which let the SDK bridge drop a `map_err` it had been carrying. **A test I wrote wrong found the better fact.** I asserted that exactly one house per chart has one planet for both lords; it is two, because the **first** house's lord *is* the lagnesha by definition. So a question about the native's own self can never be one of the fourteen pair judgements — 3 899 of 25 908 matters, 15.0%, registered as C113 and **reported** as `same_lord` rather than returned as an empty list that would read as *nothing holds*. The count decomposes — 2 159 charts + (2 159 − 419 luminary-ruled lagnas) = 3 899 — and that decomposition is a **hard failure** in the pass, not a printed sentence: proved red by miscounting the first house on purpose, `check-muntha` exit 1 naming both figures. **The twelve unbuilt yogas are named, not absent.** `YearYoga::ALL` holds all sixteen, `awaiting` says what each of the twelve still needs and is matched exhaustively so a yoga cannot be added without a decision, `unanswered` lists them at every call, and `holds` returns `Option<bool>` because *did not hold* and *cannot be told* are different statements. They do **not** cross the boundary yet: crossing a per-matter answer means deciding which matters a caller asks for, which is the residence decision's shape again and its own unit. Next: the ones that need a third planet's aspects and Tajika's malefics — Manau, Kamboola, Khallasara. |
| 2026-09-23 | **The two corrections Table X-3 found are made, and making them opened a third question worth more than either.** C110's degree is on the Ishrafa, C111's sign's-end kind ships as the table's **Bhavishyat** (the prose's *Rashyanta* survives only in `RASHYANTA_DEG`, which names a **position** and not a yoga), and the **Poorna** the table adds is built: the Ithasala now has the three kinds the table enumerates, and `Yoga::is_ithasala` is the question the other fourteen yogas actually ask, asked once. **The corrections do not meet.** The table begins Ishrafa a whole degree past and gives Poorna as a narrowing of *Vartamana*, which it states only for a faster planet **behind**; between them is a band the table bounds twice and places once. I could have picked a side in a line of code. Instead the sweep counted it: **934 of the 29 166 aspecting pairs** over the recorded births' first forty years — a twelfth of every Ishrafa, and the three candidate answers are *the most fulfilled thing a pair can be*, *generally unfavourable*, and *nothing at all*. Far too large to settle by taste, so all three ship as `SubDegree` in a `DrishtiRules`, with `_with_rules` twins beside `between` and `drishtis` as the SDK already does for readings, and `Between::disputed` marking the pairs so a consumer can show a judgement as contested. **The default is argued from the table and not from astrology**: Poorna either side is the only reading under which the degree the table prints does any work at all, and the measured sizes agree — 940 pairs in the Poorna the table states outright against 934 in the band, sitting symmetrically either side of an exact aspect. A reading on which one side of exactness is immediate fulfilment and the other is nothing would have to explain that asymmetry; the book does not. Registered as C112, with C110 and C111 closed by choice and their actions rewritten to what actually shipped. **The sweep made me account for what it could not reach.** It first died on `c028-troms-1988-06-21` — a birth above the polar circle in its own summer, whose annual chart has no sunrise to divide a day by, which the SDK refuses rather than inventing. A `continue` would have hidden it. §9 now states that 55 births times 40 years is **2 200 years asked for**, and names the whole of the difference: 40 years that never returned from the search at all (`c048-kathmandu-2399-12-30`, past the built-in ephemeris's span) and 1 that returned but could not be founded, with the SDK's own words for why. 2 159 + 40 + 1 = 2 200, so the percentages are read as what the charts hold and not as what the corpus holds. **A shipped enum moved through four bindings and the boundary**, which is why this went alone: `TsTajikaYoga` regenerated into C, Node, Dart and Python; both enum canaries moved with their reason on the line; the hand-written `TajikaYoga` union in Node's `.d.ts` was updated by hand because nothing generates it; and the Node binding test now asserts each of the four kinds stands where its own degrees put it **and** that the corpus reaches all four, so a variant that stopped crossing would be caught by more than a member count. Next: the lagnesha and karyesha pair, and the four yogas that need only the aspects. |
| 2026-09-23 | **The sixteen yogas are read and designed, and the reading corrected two things already shipped.** Table X-3 was read off the rendered pages, and it settles the module's shape: **fourteen of the sixteen are not facts about a chart**. They are judgements about a pair — the lagnesha and the **karyesha**, the significator of the matter asked about — so \"what yogas does this year have?\" is not a well-formed question and the API takes a matter. Only Ikabala and Induvara, which say nothing about a matter, are chart facts. Everything the other fourteen need is already built: the aspects, the five-fold strength, the Hudda, Drekkana and Navamsha lords that \"unqualified\" turns on — the book defines that term precisely — and the chart's own dignities. **Two corrections to shipped code** came out of it, both the book disagreeing with itself: its table wants Ishrafa a whole degree or more ahead where its prose says only ahead within the orb (C110), and it calls the sign's-end Ithasala **Bhavishyat**, one of three kinds, where the prose calls it Rashyanta and a kind of Vartamana (C111). A third kind, **Poorna** — within one degree — is not built at all. Those three go first and alone, because they move a shipped enum. This is the pattern the project has used since Phase 4 paying off: the page was written before the sixteen rules were written on top of a reading that was wrong in two places. |
| 2026-09-23 | **The Tajika aspects are built, and the word \"aspect\" turned out to mean two things.** One is a relation between **signs** — friendly at 3, 5, 9 and 11, inimical at the kendras, nothing at 2, 6, 8 and 12 — which is what the year lord asks for and what disqualifies its strongest claimant. The other is the **deeptamsha**, a distance between **planets**, each with its own orb and a pair governed by the *mean* of the two. The orb table had carried a \"verify\" in the research page since 2026-09-04 and is now read from a text, the mean rule with it. **The thing an implementer would get backwards**: \"behind\" is degrees **within the sign**, the completed signs deleted, so the source's worked pair has the Sun at Leo 3°50′ behind Mars at Scorpio 7°42′, three signs further on — a test asserts exactly that, because longitude says the opposite. A planet at 29° acts from the next sign too, which can make an Ithasala it would otherwise be past. The speed order is the tradition's fixed ranking and says so, since the source never addresses retrogression — that stays open in the checklist. **A gap in the Node declarations surfaced**: `yearLord` was missing from the hand-written `AnnualChart` and nothing caught it, because the \"declares the layer member for member\" gate measures exported *classes* on real instances and not the plain records a getter returns. A positive typecheck fixture that reads a year's answer all the way down now covers that class of miss, and it found this one. Only the pairs that **make** something cross — nine of twenty-one on the worked chart — as the boundary carries the year lord and not the strength table. 4 bindings agree on 8 090 values. Next: the fourteen other yogas, which are compositions over these pairs. |
| 2026-09-23 | **The year lord crosses, and it crosses with its reasons.** `varsha_json.varshesha` names the three readings; each year's chart answers `year_lord`, the step that chose it as a `TsVarsheshaChosen`, its exact strength and whether the Moon stepped aside, with every claimant in a **ragged** `year_claims` section counted by `claim_count`. A planet alone cannot say whether it holds the year on strength or by a fallback, so the boundary carries the reckoning and not just the answer. **The strength crosses as an integer** — sub-sub units, 3600 to a unit, in an `i32` — because the source's own worked chart separates its first two claimants by 25 sub-units, and each binding turns it back into units, sub-units and sub-sub with a `toString` that writes it as the sources do. The seven-planet strength table itself does **not** cross: it is what the reckoning was computed from, a Rust caller has `sdk.chart().panchavargiya`, and five columns per planet per year would be for a reader nobody has. Two canaries moved as they should, the Node and Python enum counts, each with its reason written beside it. All four runners print the lord, its step, its strength and every claim: **4 bindings agree on 8 018 values**. The three examples print the year lord with its claims and are byte-identical — checked by `diff` this time, which is what the last entry's correction was about. Next: the Tajika aspects. |
| 2026-09-23 | **The lord of the year is built, and the source's own worked year is the case that proves the obvious reading wrong.** "The strongest of the five office-bearers" is not the rule. In the book's chart Jupiter leads at 14:46:00 and stands in the **second** from the annual lagna — a neutral house, which in Tajika gives no aspect at all — so the book disqualifies it in as many words, and the **Sun** takes the year at 14:20:15. Saturn is stronger than any of them (16:47:45) and holds no portfolio, so it never enters the reckoning. **I had written the test expecting Jupiter and the code answered the Sun**; the code was right and my expectation was the guess, which is why the acceptance test is the book's worked value and not my reading of the prose. Reading the passage that settled it turned up two rules I did not have: the **Moon is passed over** — "unable to govern unless extraordinarily strong" — even when it is the strongest aspecting office-bearer, and an outright tie goes to the Dina-Ratri Pati under a second reading. Both ship as knobs beside the lagna-lord one, so all three of the book's recorded disagreements are named rather than silent. The answer carries **why**: which step of the chain fired, every claimant's strength, portfolios and aspect, and whether the Moon stepped aside — because a year lord reached by a fallback is a different statement about the year and the planet alone cannot say so. The aspect this rule needs is a relation between two **signs**; the deeptamsha orbs the sixteen yogas want are a different module, and the page says so rather than implying the aspects are done. Next: the crossing, then those aspects. |
| 2026-09-22 | **The Panchavargiya bala is built, and all forty-nine figures of the source's own tabulated chart come back.** Five parts — Griha 30, Uchcha 20, Hudda 15, Drekkana 10, Navamsha 5 — each reduced by how a planet stands to the lord of the division it is in, quartered into a Vishwa bala out of twenty. **The arithmetic is exact**: the source writes a strength as `14:20:15` and says to work to sub-sub units, so `Bala` holds integers, 3600 to a unit, and only the Uchcha truncates, because the source's own method says to ignore that remainder. Floating point would have lost the last figure of every printed cell. **Three things are Tajika's own**: friendship is positional and read off the annual chart, so a planet sharing a sign with that sign's lord is its **enemy** — which the worked chart turns on, Mercury and Venus both in the Sun's Leo taking a quarter each; the decanate lords run in one cycle from Mars, `(sign + 5 × decanate) mod 7`, reproducing all 36 printed cells; and the Hudda, which the book calls patternless, ships as printed with its three transpositions from the Egyptian terms named, held by two invariants over all sixty cells because the book's own example touches seven of them and none of the three. Every cell is reproduced from the chart **the SDK founded**, not the source's longitudes, and through the façade: the strongest of the seven is Saturn as the source has it, and the strongest office-bearer, which is what the year lord is chosen by, is Jupiter at 14:46:00. The navamsha comes from `teistro_vargas` rather than being written again. Next: the Tajika aspects, the other half of the year lord's rule. |
| 2026-09-22 | **The office-bearers cross, and the residence question is answered by not answering it.** `varsha_json.place` is optional: absent, no chart is founded and the answer is what it was; `\"birth\"` is a word the caller writes rather than a default the caller receives, since the one text read casts every chart for the birthplace — a year its native lived elsewhere included — without stating a rule; and a residence is written in the place shape each binding's own `found` takes, which each layer turns into the boundary's words. Each year's chart comes back in `annual_charts`, **row for row beside `praveshas` or empty**, and each binding reads it as `pravesha.annual`, refusing a section of any other length rather than pairing a year with another year's chart. **One DX defect the first refusal showed**: the strict reader, handed the whole record, could name only `varsha_json`, so the place is read on its own under `varsha_json.place` and every refusal of it names that field. And the Node layer at first threw its own `TypeError` for `'home'`, so a wrong word got different words in each binding; a word now crosses as written and the SDK refuses it the same way in all of them. All four parity runners print each year's lagna, part of the day and five lords: 4 bindings agree on 7 730 values. **A claim of mine was wrong**: the annual chart's three examples did not print byte-identical output, because the Python one printed the error and its `__str__` adds the field on a second line; it prints the message now, and measuring all eleven shared examples found 4 identical and 7 differing in presentation, which is `## Next` item 2c rather than a sentence. Next: the Panchavargiya bala. |
| 2026-09-22 | **The five office-bearers are built, and the source's own worked year reproduces through the façade end to end.** `teistro_tajika::office_bearers` reads them from a `YearCharts` of six named numbers, and `sdk.chart().office_bearers(&natal, &annual, years)` fills it from a birth chart and an annual chart the caller founded; the Tri-Rashi lord ships as the rule, the 24 printed cells its test. The source works one birth — Bombay, 20 August 1944 — to its forty-first year, and the SDK lands on its return to **1.5 s**, its annual lagna to **0.9′**, and all five of its office-bearers. Reading it back found two things the text does not say outright. **Its return is the mean one**: its Dhruvanka is whole mean sidereal years modulo a week, to the second, so `Reading::Mean` is what matches, and the true return is the \"few minutes\" it sets aside — 0.83 on a mean ayanamsha, 4.84 on a nutated one, the difference being nutation. **Its positions are geocentric**: the Moon was 58′ out on the topocentric conformance profile, which I took for an error until the Moon's parallax at Bombay turned out to be the whole of it; on the geocentric default it is 2.7′. My own first bound was wrong too — I had written \"a few minutes\" as at least one, and it is 0.83 — so the test now asserts the source's claim and not a figure of mine, and the page states the measured worst rather than calling it a rounding. The office-bearers **do not cross yet**: three of five need the annual chart's place, and crossing them is the residence decision. Next: that decision, then the Panchavargiya bala. |
| 2026-09-22 | **The Muntha is built from the first Tajika text this repository has read, and reading it corrected a field I had just shipped.** The rows under Tajika had been written from the references' feature lists; K.S. Charak's *A Textbook of Varshaphala* (rank 2) is the first text, and its tables came out of the OCR as noise, so each was read off the rendered page. **Three of the four tables the year lord needs turned out to be rules**: the Tri-Rashi lords are the Dorothean triplicities under a positional rule (0 of 24 cells disagree), the Tajika Drekkana is one expression, `(sign + 5 × decanate) mod 7`, reproducing all 36 printed cells and all seven worked values, and the Hudda — which the book calls patternless — is the **Egyptian terms**, every degree column exact and 57 of 60 lords, with three signs transposing two lords that the book's own worked example never touches (C106–C109). **The field.** `Pravesha.year` shipped documented as *\"which year of life it opens: 1 is the first birthday, so a reader's age during that year is year − 1\"* — two claims in one sentence, copied into the schema and all three bindings. The arithmetic was right; the words were not, and the Muntha hangs off exactly that number. Fixed and pinned against the rival before it merged. **The Muntha** needs nothing but the birth, so it rides in the `praveshas` section beside its return and all four bindings agree on it, value for value, on 7 514 values; its argument is named `completed_years`, because over 55 recorded births and 120 years the rival reading never agrees on the sign and agrees on the **lord** 550 times in 6 600 — one in twelve, only at Capricorn→Aquarius, where Saturn rules both — which is invisible often enough to pass a spot check. **The corpus records no Muntha but records its only input**, the lagna, so the pass holds 6 655 of them against the *recording's* lagna, none wrong. Its first draft priced a lagna's risk as the worst error anywhere over the tightest margin anywhere, **74%** of the way to moving a Muntha for life; they were two different births, and per birth the worst spends **0.03%**. The other four office-bearers cannot ride with the instant, because three of them need the place the boundary deliberately leaves to the caller. `births` moved out of the annual chart's pass into a shared loader, its page byte-identical after. Proved red by switching to the rival: gate exit 1, and the three tests that should fail did. Next: the office-bearers over a founded annual chart. |
| 2026-09-22 | **The annual chart crosses, and the gate that should have caught two of my mistakes reported success instead.** `TsChartRequest.varsha_json` names the reading and the years; the instants come back in section 35 `praveshas`, ragged by `cast.pravesha_count`, and Node, Dart and Python each take a `varsha` request and read `chart.praveshas`. **The boundary answers the instant and not the chart**, which is the residence crux respected in the API rather than decided by it: the caller founds the year with the place they mean. Each binding gained a worked example. *(Corrected on 2026-09-22: this said the three print byte-identical output, which was checked by eye and was not true — Python's refusal line carried a second `field:` line, because the example printed the error rather than its message. It is identical now, and a measurement of all eleven shared examples is item 2c of `## Next`.)* **The finding is `check-parity`.** It printed `ok` and exited **zero** while the Python runner crashed — *\"2 bindings agree\"* where there should have been four — because `run` printed `FAIL the Python runner failed` and returned `None`, and nothing ever counted the `None`s. A runner this machine has no toolchain for is a legitimate skip; a runner that was tried and **failed** is not, and the gate could not tell them apart. It counts attempts now and a lost runner fails it, proved red by making the Node runner throw. That is the `^FAIL`-grep sweep again in another costume: **a detector that cannot see the failure reports success** — and this time it was hiding two of my own defects, `years.charts` in two runners whose batches are iterables. Two more came out of building it: the Dart `found` took `varsha` and never forwarded it to `foundMany`, which its own test caught as zero returns; and the Dart test provider covers only about 2000 to 2050, so a 1990 birth correctly answered **zero**, which confirmed the coverage cap rather than exposing a bug. All four bindings now agree, value for value, on 7 289 values including three readings over twelve years of two charts. Next: the Muntha and the lord of the year. |
| 2026-09-22 | **The annual chart is built, and the pass moved onto it without moving a number.** `crates/tajika` holds the Varsha Pravesha; `sdk.chart().praveshas` gives the instants and `sdk.chart().annual` founds the chart a year opens. The three readings are named members rather than a default and a fallback — `Sidereal` (the tradition's), `Tropical` (the Western return, which a consumer asks for and nobody gets by accident) and `Mean` (the older arithmetic, and **the only one that needs no ephemeris**, so it answers from a context with none). Every answer carries the reading that made it, so a stored instant says what it is rather than depending on the settings that read it back. **The pass was rewritten to call the shipped path and the page did not change by a byte** — which is the only evidence that the thing measured and the thing built are one thing, and the reason a pass must never keep its own copy of what it measures. Two DX defects came out of building it. A year past the provider's coverage used to surface the provider's own `OutOfRange` naming an instant the caller never mentioned; the façade caps to the coverage and **fewer instants is the answer**. And capping had to come **after** the refusal, not before: a nonsense `through` of zero capped to zero would answer an empty list where the caller wanted to be told, so `teistro_tajika::years` is public and both callers refuse through it. Two lints earned their keep on the way: `target-declares-the-feature-it-needs` caught the new test using the built-in ephemeris without saying so, and `crate-is-listed` caught the crate table. The Rust surface grew by two entry points, both exercised, and the unreachable count stayed at eleven because the façade re-exports what the new signatures name. Next: the Muntha and the lord of the year. |
| 2026-09-22 | **Phase 7 begins with the annual chart, and the pass falsified the first shape of it by 18 arcseconds.** The Varsha Pravesha is the instant the Sun returns to where it stood at birth; everything in Tajika hangs off it and four catalogued dashas wait on it. **Research first, and it found the module needs no new solver**: a return is the crossing search this SDK already has, `Quantity::Longitude(Sun)` over a lattice of one line, and the monthly chart is the same search with a step. What it does need is a decision the tradition makes and a frame does not. **A chart's frame is tropical and its ayanamsha is an offset**, so "the Sun returns to its natal longitude" names two instants — and forty years on the tropical reading is **14.2 hours** from the sidereal one, which puts the lagna **177°** away: the opposite sign, a different lord, a different reading of every house. The mean-motion rival is 14.5 minutes and 5.6°. Both are measured as rivals and priced in **lagna degrees** rather than minutes, because degrees are what a reader reads. **The defect the pass caught is the one that could not be caught by reading the rule.** A `Frame`'s own `Zodiac::Sidereal` applies the **mean** ayanamsha and a founded chart applies the **nutated** one — 18.46 arcseconds apart, an entry this tracker already carried — so the first shape searched the frame's sidereal longitudes and the read-back through a founded chart falsified it at **18.363 arcseconds**, about seven minutes of the Sun's time and two degrees of lagna. It reads **0.000** now. Two rules came out of it. **Read the answer back through a founded chart and never through the source the search used** — a solver asked whether it landed on its own lattice line will always say yes. And **the shift belongs in one place**: `teistro_astro::sidereal` now holds the `Sidereal` source and the `Zodiac` it reads in, which were the panchanga limb search's private adapter and are the annual chart's second consumer; a second copy would have been a second thing to get 18 arcseconds wrong. **The step is the whole cost and taking it found another defect.** The default one-day step samples forty years of every birth: 3m19s. The Sun never turns, so a ten-day step cannot pass the one target twice — 40s, and the page is **byte-identical** at one day and at ten, which is the check rather than the claim. It then failed on a birth at the ephemeris's edge, because the search brackets a crossing *between* samples and reads a step below where it was told to start; the window starts a step in, and a birth whose first return is past the ephemeris is counted as the ephemeris stopping rather than as a return lost. Proved red by setting the basis back to mean: 18.363 arcseconds, falsified. Next: the module itself — the Muntha, the Varshesha, and the four dashas this unblocks. |
| 2026-09-22 | **A consumer could register a nakshatra-seeded dasha and not a sign-based one, and nothing said so.** The coverage page written the same day found it by asking a question it had not been written for — not *why is this not built* but *who can supply it* — and the answer was 3 of the 22 by a consumer and 19 by nobody, with `STHIRA` and `VARNADA` in the second group **only because there was no definition for them to arrive as**. That is the distinction the no-dead-ends mandate turns on: the text being unsettled blocks *this* build, a missing definition blocks *everyone*, and only the second is the SDK's to fix. `RashiDefinition` closes it. `RashiRow` became data — a `DashaName` and a `Cow<'static, [u8]>` where it had a `DashaSystem` and a `&'static [u8]` — so a row is shipped or owned with one kernel for both, and a shipped row still clones **without allocating**, which the allocation tests hold. `DashaDefinition` is one type over the two kernels and the registry is one registry, because a consumer registers *a dasha system*: two registries are two things that can disagree about whether a key is taken, and a catalogued key must be refused whichever kernel asks. **The kernel is stated and never guessed.** The two rows are structurally disjoint, so a reader could tell them apart by which fields are present — and would then read a row with a typo in `lords` as sign-based and refuse it by a field the caller never wrote. One `kernel` tag buys the refusal that names what they did write, and an untagged row is refused by `kernel`. Proved by twins, which is the strongest thing a registration test can assert: a consumer's `ACME_CHARA` answers **period for period** as the catalogued Chara, in Rust, across the C boundary, and in all three bindings. Node takes a discriminated union, Python a `Literal`-tagged `TypedDict` union, Dart a **sealed** class — and the Dart and Python type-checkers both refused the first shape, which is the gate doing its job before CI. Two things fell out. The rust-surface pass caught `DashaDefinition` being named in a signature and unreachable from the crate root, so the façade re-exports it and its two arms, and that page's falsified count went **12 to 11**. And the coverage gate measured its own repair: 3 and 19 became 5 and 17 on a page nobody edited. |
| 2026-09-22 | **The catalogue names forty dasha systems and this build computes eighteen, and nothing said which twenty-two were missing or why.** The gap itself is not a defect — a catalogue is a key space and a build is a set of kernels (Q38) — but an **undeclared** gap is, because a consumer reading the catalogue cannot tell a feature from a name waiting for one. It was half-declared: the façade already refuses an unbuilt system by name and hints with the built ones, and `dasha-kernels.md` examined every system and marked it V, T or S. What nothing did was hold the two together, and the design page's own verdict table says **56 systems examined** beside a catalogue of 40 and a build of 18 — three numbers in three places, which is the shape this repository keeps finding in its own prose. `dasha-coverage-measured.md` (`check-dasha-coverage`, the fast check's fiftieth gate) counts both sets from the types and holds the difference against an exhaustive reasoned list, **grouped by the blocker that would have to go first**: eleven wait on a text (Shashtihayani's received lords sum to 69 and not 60; the tithi, yoga and karana seeds have no reference index because the cycles are 30, 27 and 60, not 27), five on the kernel (Tara, Karaka and Ashtakavarga are **one** mechanism asked for three ways, which is why ADR-0017 takes them together or not at all), six on a module they are the last step of (Aayu on longevity; Sudasa on the karakamsha; Mudda, Varsha Narayana, Varsha Yogini and Patyayini on the annual chart, which is Phase 7's). Two of the twenty-two are nearer than the page expected and the writing found it: **Varnada's and Sudasa's lagnas are already built** in `teistro_points`, so what blocks them is which school's variant the dasha counts from, not the point. The list fails **three** ways — a system neither computed nor listed, a reason that outlived its blocker, a reason naming a system the catalogue does not — and each was proved red separately. The first shape put the three checks in front of the refusal pass, so two of them could fail the run before their own counts ever reached the page: a claim that can only ever read zero. They are measured now and only the systems that pass them are asked for. And **"declared" is measured rather than asserted**: all 22 are asked of a real founded chart and every one comes back refused, naming the system, the field `dashas[0]` and every system the build does compute. A member that answered an empty reading would be indistinguishable from a bug, and nothing but a call can tell those apart. And the page asked one more question than it was written for: **who \*can\* supply the twenty-two?** `DashaSystems::register` takes a `UduDefinition`, so the three rows waiting only on a citation can be registered today by whoever holds the text — proved by registering the design page's own stated Shodashottari row under a demonstration key and watching its 116 years fall out of its lords. **The other nineteen can be supplied by nobody**, because there is no `RashiDefinition`: a sign-based system a consumer has the text for is as shut to them as to this build. The text being unsettled blocks *this* build; a missing definition blocks *everyone*, and only the second is the SDK's to fix. Next: Phase 7, whose queue this page just made explicit. |
| 2026-09-22 | **The Hindi gap is 162 units and one command, not 118 strings to guess.** The tracker has carried "`hi-Deva-IN`'s 69 messages" as a pending item for days, and the reviewer section said plainly that the roadmap asks for `hi` sign-off and there is not a sentence in it to sign off — true, and it left a reader with no next step. Two things were wrong with the framing. The number is **118** messages, not 69: 69 counted the ones with *words of their own*, which is the reviewer's load for a locale that already has them and not the translator's for one that has none. And the work does not belong in this repository at all: `teistro-intl export xliff` has written XLIFF 2.1 with the base locale as source and a note naming every parameter since the intl crate landed, and `import` brings it back. Measured from the exporter rather than counted in prose: **1512 units a locale, 162 left to translate** for `hi-Deva-IN`, `sa-Deva` and `sa-Latn`, and 8 for `ne-Deva-NP`. The page now prints the figures and the commands, and the commands were **run** before they were printed — which caught the printed `import --file`, since the real flag is a positional. What was missing was a gate on the path the maintainer is about to walk. `what_was_exported_imports_back_unchanged` walks `ne-Deva-NP`, which is complete, so the **empty-target** path is never taken in it; the four-unit `sa-Deva` miniature takes it once. The locale with 162 of them is the one that matters, because an import that wrote empty targets would blank a locale's records — the shape of the bug `load_pack` already had once — so the round trip is now held on `hi-Deva-IN` at full size, asserting the locale is byte-identical after a return and that nothing was invented. Two defects came out of writing it. `fill` wrapped **fenced code blocks** as prose, which mangled the commands into one line — and had already mangled the altitude bound on `horizon-absence-measured.md` into unreadable soup, where it has sat since that page was written; the fix is three lines and repairs both. And the XLIFF round trip was behind the `cli` feature, so a tool that wants to *report* how much of a locale is left to translate had to take `clap` with it: `xliff` is its own feature now, `cli` enables it, and no consumer's build changes. |
| 2026-09-22 | **The last of the four queue rows called a decision was one, and it is decided.** `ASHTAKAVARGA` survived when `BHAVA_BALA`, `VIMSHOPAKA` and `DASHA_PHALA` all dissolved on reading their sources, and what it asked was never whether the numbers cross — they have crossed as `Document.ashtakavarga` since the module landed, eighty-four bindus and a twelve-place sarva and two reduced rows and three pindas a graha. It asked **which of them a sentence is worth**. The answer taken is the two a text quotes: each graha's bindus in the **sign it stands in**, and each sign's **sarvashtakavarga**, said once rather than repeated inside every graha's sentence. Nineteen items, seven and twelve. It is the first composer to take **two inputs**, and for a reason the row had predicted: an `AshtakavargaReading` is indexed by sign and knows nothing of where the grahas are, so the chart stands beside it. **No band and no verdict**, which is `bhava_bala`'s rule and not `strength`'s — a `GrahaShadbala` carries `required_rupas` and an `AshtakavargaReading` carries no threshold at all, so *weak* and *strong* would be a rule invented rather than said, which is the very objection that keeps `shadbala-strength`'s four bands out of the state corpus. A rule worth holding in both places or in neither. What it leaves is left because it is **ruleset-dependent**: `ashtakavarga-measured.md` falsifies the text's reading of the trine reduction, the Ekadhipatya and all three pindas against the corpus on all 77 charts, so a sentence saying one would have to name the ruleset it was reduced under — a different sentence and a decision of its own. The sarva is said by sign, which is what the reading is indexed by; by bhava would fold in the house system, which is `houses`'. Two messages in both strict locales, declared and **exercised** in all three bindings, and the founded chart's plan grew by 38 renderings. The gates named every wiring point before the tests did: six binding declarations, the founded-chart pass and the section table, each refusing in turn. One rot caught on the way: the section paragraph said *"two share one blocker — `VIMSHOPAKA` and `VAISESHIKAMSA`"* and `VIMSHOPAKA` had been built for a day, so the rows nothing says are now **derived and named** rather than described — `POINTS` and `VAISESHIKAMSA` — and the counts that stood in the tracker's own prose are deleted rather than updated, having moved three times in one sentence. |
...
| 2026-09-22 | **Half the composers' surface was unmeasured, because a zero is a question and nobody asked.** `interpret-measured.md` counted **33 of 65 keys at zero**. The cause is one sentence: the recorded corpus keeps bodies and chara karakas — no graha states, no almanac, no strengths — so nine of the thirteen composers run over it and four do not, and the zeros are a fact about the fixtures rather than about the composers. Nothing said so, and reading them as nothing-to-see is how `sdk.phala.tithi` sat at zero for a week while eleven shipped rules could not hold. Every key is now emitted by **one chart the SDK founds itself**, with every section asked for and both corpora loaded through the same `load_pack` a consumer uses: **64 of 65**, and every item rendered in both strict locales — **786 renderings**, no fallback, nothing to warn about. The one key the chart cannot reach is `sdk.condition.combust` — no graha of it is burnt — and it is named with that reason. The list fails three ways: a key neither emitted nor excused; a key excused that the chart emits; and an excuse naming a key no composer emits at all, so a rename cannot leave a reason pointing at nothing. **Three of the four excuses I first wrote were wrong** — the undecided lajjitadi, the dasha's favour and the birth by day were guesses about what a chart would contain — and the check refused them one at a time. That is the argument for the check rather than the list. |
| 2026-09-22 | **Eleven shipped rules could never hold, and building a composer is what found them.** Q39 asked whether a plan should say the almanac's own limbs. Answering it *yes* meant asking a document for the limb running at the chart's instant — and `RuleInputs::of` built its chart with `panchanga: None` from **every** document, while `with_rule_inputs`, whose whole promise is to ask for exactly what the rules read, never asked for the almanac. `Rule::reads_panchanga` had been there the whole time to ask with. So the 11 rules that read a limb — six arishtas, all four gandantas, one computed dosha — resolved to *"needs a tithi, and the chart has none"* on every chart: honest, permanent, and a **dead end**, because no consumer could give them one. `phala`'s four limb readings were unreachable for the same reason, which is why the measured page has counted them at zero since they were written and nobody asked why — a zero on a generated page is a question, and this one went unasked for a week. The fix is two lines and a helper: `with_rule_inputs` asks for the section when a rule reads a limb, and `birth_limbs` reads the running limb out of the day's spans, with what it can and cannot answer stated where it lives — the tithi's and nakshatra's ghatikas come from the limb's own interval, the rising sign's do not because the lagna moves at its own rate, and `on_sankranti` stays false because *falls on a sankranti* is a window the chart's maker reads. The almanac also gained `yoga_at` and `karana_at`: two of its four moving limbs could be asked for at an instant and two could not, with `span::at` already there for both. A test holds that all 11 are answered rather than excused. `panchanga`, the thirteenth composer, says the five running limbs, the Moon's pada and whether the birth fell by day, and the tithi says its paksha because the entity names split what the key joins. `MEMBERS` 13, `KEYS` 65. |
| 2026-09-22 | **verify went red on all five platforms for a generated type nothing declared, and it is the Dart keyword's shape exactly.** An entity kind's key union was read from `sdk.entity` — the right source for a member's **name** and the wrong one for its **key**, because a closed catalogue kind's members are a fact of the catalogue and a locale naming none of them does not make the kind smaller. `sdk.phala.state` renders the `phala` form of `state.COMBUST`, a kind no locale names because the vetted tables have no word for being burnt, and every emitter wrote `state: StateKey` against nothing: `messages.d.ts(683,36): error TS2304: Cannot find name 'StateKey'`. `check-intl` was green because the files were **up to date**. Two fixes, one for the defect and one for the class: the model seeds a closed kind's members from the catalogue whenever a message names the kind and the locale names no member of it; and `check-intl` reads every `<Kind>Key` back out of the TypeScript, Dart and Python it has just written and fails on a reference it does not declare. Proved red on all three targets by removing the first fix. The cost of *not* having that guard is the measure of it: five red verify jobs and forty minutes, for something a three-second textual check sees. |
| 2026-09-22 | **Q39 opens, and the count of open questions is refused in prose.** The section table raised the question: `PANCHANGA` is said only where a corpus carries a reading, so the almanac's own limbs — and the karana, which no composer mentions in any form — are computed, named in all five locales and said by nothing. Every name a composer would need is bought, so the cost is a frame a message; whether a **plan** should recite them is a design call, because this page has twice declined a message for being the wrong *kind* of thing to say — `exactLongitude` renders a longitude alone and is a fragment a consumer formats with, and `rashiNature` is a fact about the zodiac where every chart would say the same twelve sentences — and **neither reason holds here**. Recorded rather than decided, with three options and a recommendation, because it is a third `PlanRequest` member in a day that has already spent two. The second half is the rot it exposed: the resume step's first sentence claimed an empty register when one question was open, and then a single one when two were. That is the crate table and the corpus size again, in **the first thing this document tells a reader to trust**. `open-question-is-named` requires every question the register marks `open` to be named in the tracker, and refuses any phrase that counts them at all — three phrasings cover every count without reporting one occurrence twice. Both halves proved red, and two history sentences that quoted the old wording were reworded rather than exempted, because an exemption is where a lint starts to rot. |
| 2026-09-22 | **`planet-condition` closes, and nothing cheap is left: 214 readings reached no reader this morning and 47 do now.** Its seven key onto `dignity.{EXALTED, MOOLTRIKONA, OWN_SIGN, DEBILITATED}` and `state.{RETROGRADE, COMBUST, VARGOTTAMA}` — exactly the conditions `conditions` already names — so that composer gained the vocabulary parameter every corpus-reading composer takes and says the reading **beside** the fact rather than instead of it: the fact is the SDK's and the reading is the corpus's, and a graha that is not burnt says nothing of burning however much a pack carries. It could not go in `states`, because a `GrahaState` carries no navamsha and cannot tell whether a graha is vargottama. **`state`'s own members have no name in any locale and that did not block it**, which is the distinction this day kept getting wrong in the other direction: an unnamed kind blocks a composer that would print its *name*, not one that prints a **form** on its record. What is left is 39 readings in four remedy families that want Phase 8's `remedies` to decide which graha or rashi a remedy is *for*, 5 `special-lagna` on a vetted name table, and 3 `inauspicious-kaal` on something that reads a time rather than a chart. `KEYS` is 58. |
| 2026-09-22 | **The full gate sweep was run in the wrong shell and could not fail.** My own notes say it: run the loop over every `check-*` in `fast-check.yml` under **bash**, because zsh does not word-split `$(...)`. I wrote the loop and ran it in zsh, where `for g in $gates` iterates **once** with all 49 gate names as a single argument — `cargo xtask "check-absence check-accuracy …"` — whose error message does not match the pattern the loop greps for. It printed *0 failures* on a tree that had two stale generated pages, and fast-check found one of them on `326c990` and one that had not been pushed yet. Under `bash -c` the same loop found both immediately. The lesson is not about shells: **a check that cannot fail is worse than no check, because it is believed**, which is the same shape as a gate going green for the wrong reason. Both pages moved for the reason the sweep exists to catch — the twelfth composer's ten messages grew `i18n/`, which a gated figure on the records page reports, and its Rust façade method added a member the exercise page counts. |
| 2026-09-22 | **The native-review set was a flag in two sentences, and it covered a tenth of itself.** Phase 6's exit criterion asks for `ne` and `hi` sign-off, and what that covers lived in two design-page phrases written when two namespaces existed — and ten new messages were written today alone. It is measured now, and the measurement is the right shape: a message that only renders a catalogue value carries **no prose**, because a reviewer reading `{$graha :entity kind=graha}` is reading `sdk.entity`, while a message with words in it is what someone has to read. The **parser** decides which — literal text is `Part::Text` and a pattern's other parts are expressions and markup — so a message rewritten from one into the other moves the list by itself, where a regular expression would have needed maintaining. **69 Nepali messages say words of their own**, over six namespaces, against the two the sentences named; the largest is `sdk.calendar` at 28, never once mentioned as awaiting review. And `hi-Deva-IN`, `sa-Deva` and `sa-Latn` carry no `sdk.*` message at all, which is what `base` completeness means, so their share of the sign-off is the **names** and `entity-names.md` §4 already holds those. The roadmap asks for `hi` sign-off and there is not a sentence in it to sign off. |
| 2026-09-22 | **The twelfth composer, and the section table asked for it.** `STATE` had read as *answered* on that table because `conditions` says every fact a `Placement` carries — nine of them, all said — while the section's own `GrahaState` carries a dozen and the other half reached no reader at all. That is why the table's third column means **what is left** rather than *why nobody says it*, and it is the first row whose caveat turned into a composer. `states` says how a graha stands to its dispositor under all three friendships, and the four avasthas — the fifth of its sign, its wakefulness, its brightness where the chart decides one, and the lajjitadi. Almost every word was already bought, which is the opposite of `dasha_phala` and why this followed rather than led: `Relationship` and the four avastha kinds are catalogue members that all five locales name, so it is six messages and not one new term, with nothing for the `ne`/`hi` review that it has not already seen. What is *not* said is not said by **name** — the Sayanadi and the Cheshta are catalogued and unnamed, the vetted tables stopping at the four — and the row cites them. The **undecided lajjitadi are an item of their own**: `Lajjitadi` names what holds, what is ruled out and what nothing in the chart decides, and a plan printing only what held would turn *we cannot tell* into *no*, which is the silent default this project refuses. Declared and exercised in all three bindings. `MEMBERS` is 12, `KEYS` is 56, and the state corpus's 23 avastha readings reach a reader, taking its shortfall to **54** from 214 this morning. |
| 2026-09-22 | **137 state readings reached a reader today; 214 became 77.** Three steps, each cheaper than the last, and each chosen for what it did not need. `dosha-timing` (62) needed no new subject: it keys onto rules the `readings` composer already walks. What the birth nakshatra *is* (57) needed no new computation: the catalogue already carries every nakshatra's gana, nadi, yoni, varna and element. `dasha-lord-effect` and `dasha-lord-activation` (18) needed no new name and no decision — only the **dasha phala section**, which is what makes the reading this chart's rather than the zodiac's. That last one is the distinction worth keeping: a reading of a graha *as a dasha lord* has nowhere to hang until something says what that graha's dasha will be like, and saying all nine unconditionally would have been a table dump — the same objection that still holds `graha-color`, `graha-direction`, `mantra-ritual` and `ishta-devata` back, since *which* graha or rashi a remedy is for is a rule this corpus does not record. What is left is 77: the four avastha families (23), those four remedy families (39), `planet-condition` (7), `special-lagna` (5) and `inauspicious-kaal` (3). |
| 2026-09-22 | **The eleventh composer, and the first built off the queue that found it.** `dasha_phala` says what each graha's placement says of its dasha — when in the dasha its effects come, whether its place is auspicious, the points its dignity earns and whether the placement tilts the dasha (BPHS ch. 28 vv. 7 to 10, ch. 47 vv. 3 to 6) — with the reading a loaded corpus carries of that graha as a dasha lord. The section table had named it the cheapest of the six unsaid sections and it was **still words**: `nature` is a catalogue member no strict locale names, so its place is said the way `sdk.reading.lifeClass` says a class of life — matched on the key, the words written in each locale — rather than through an entity slot that would print nothing. That is the distinction spent rather than merely recorded. The tilt is one item and not two, because vv. 5 and 6 let a placement be both favourable and unfavourable and both flags then stand, so the message carries a third arm; a placement that tilts neither way says nothing of it, as a rule nothing cancelled says nothing of its cancellation. The corpus's `dasha-lord-effect` and `dasha-lord-activation` are asked for **independently** under the forms `dashaPhala` and `dashaActivation`, which is the 18 readings this section was what the state corpus was waiting on. `dashaPhala` crosses the C boundary, is the first `PlanRequest` member spelled in more than one word, and is declared **and exercised** in all three bindings — each rendering every item it says with no fallback and no warning, which is the lesson from the pack that loaded and reached nobody. `MEMBERS` is 11, `KEYS` is 46, and the section queue is five. |
| 2026-09-22 | **Six of the eleven sections a chart document can carry are said by nothing, and the page that claimed otherwise now enumerates them.** `interpret-measured.md` carried *"the silences that remain are all of that kind"* — a computed value with no catalogue member — which was true of the **messages** the sentence sat beside and had never been checked against the **sections**. A composer says a section or it says a placement; `PANCHANGA`, `STATE`, `ASPECTS`, `HOUSES` and `SHADBALA` have one, and `POINTS`, `ASHTAKAVARGA`, `VIMSHOPAKA`, `BHAVA_BALA`, `VAISESHIKAMSA` and `DASHA_PHALA` are computed, crossed to all three bindings and said by nothing at all. The reasons differ and that is the finding: only `POINTS` and `DASHA_PHALA` want words a locale has not been given, `VAISESHIKAMSA` wants nothing but the work because its names are catalogue members already, `ASHTAKAVARGA` wants a decision about which of twelve numbers a graha deserves a sentence rather than a table, `VIMSHOPAKA` wants a knob because one composer saying four schemes would say the same graha four times, and `BHAVA_BALA` wants what a bhava's requirement is, which is not the graha rule the texts state. The list is read from `crates/sdk/src/reading.rs`, so a twelfth section fails here; a composer it names must be in `PlanRequest::MEMBERS`; and a row carrying both a composer and an excuse fails. All three were proved red before the table was believed. This is the remaining queue for the phase, and it is a table with a reason a row rather than a sentence anyone has to trust. |
| 2026-09-22 | **What the birth nakshatra *is*: 57 more readings reach a reader, and 152 became 95.** The largest single step off the state corpus's shortfall, and the cheapest. Every nakshatra in the catalogue already carries its gana, nadi, yoni, varna and element as attributes, and the corpus keys its readings onto **those kinds' own records** — `gana.DEVA`, `nadi.MADHYA`, `yoni.ELEPHANT`, `varna.MLECCHA`, `tatwa.PRITHVI` — so the subject of each is settled by the nakshatra the `phala` composer's panchanga limb already names: the Moon's, which is the janma nakshatra every text reads them from. `namakarana` is a second form on the nakshatra itself, exactly as `lagnaPhala` is a second form on a rashi. Nothing was computed, nothing was chosen and no catalogue kind had to be made permanent, which is why six messages bought 57 readings; `KEYS` is 40 and the page renders 660 readings across the strict locales with no fallback and no warning. A test states the catalogue's own answer for Bharani before asking the composer, and a second gives it another star's attributes and requires silence, because the subject is *this* chart's nakshatra and not every one. The cheapness is the test for what is left: a category whose subject the SDK already computes **and already names** can be said without a decision. `graha-color`, `graha-direction`, `mantra-ritual` and `ishta-devata` fail it for a different reason than the rest — the kinds exist and are computed, but *which* graha or rashi a remedy is for is a rule this corpus does not record, so it is Phase 8's `remedies` and not a catalogue question. |
| 2026-09-22 | **62 state readings reach a reader; 214 became 152.** The corpus's own measured shortfall was that a reading nothing says has not reached a reader, and `dosha-timing` was the one category that could close **without a decision**: it keys onto rules the `readings` composer already walks, so nothing new had to be computed and no catalogue kind had to be made permanent at the C boundary. A rule's reading and a rule's timing are two forms on one `rule.<KEY>` record — which is exactly why a reading is a form and not a record of its own — so `sdk.reading.timing` renders the second the way `sdk.reading.says` renders the first, and the composer asks for them **independently**: 34 of the shipped rules carry both and 18 carry a timing and no reading at all. Those 18 are every computed dosha, whose verse states no effect, and they said nothing in words at all before this. The message carries no prose of its own in any locale, being the entity render and nothing else, so it adds nothing to the `ne`/`hi` native review — the one place a new message usually costs something. 124 new renderings, none a fallback and none with a warning; `KEYS` is 34. The generator's table of which message says which reading is hand-kept and mirrors the composers, so it is now checked against `teistro_interpret::KEYS`: a message renamed on one side would otherwise leave the page counting those readings as *said* while rendering a key no locale carries. What is left in that shortfall needs either a catalogue decision (Q38) or a Phase 7 module. |
| 2026-09-22 | **The tracker's own crate table had gone stale by eight entries, so a lint holds it now.** `rules`, `interpret`, `dasha`, `strength`, `sdk`, `geometry`, `render-svg` and `ephemeris-builtin` — most of what Phases 4 to 6 produced — were built and this document's table of what is built never named them. Completing it by hand fixes one day; the table is a **hand-kept list beside a directory that grows**, which is the rot this repository keeps finding in its own prose, and it is the third stale claim found in this file in one day. The gate list two items above it went the same way and became a pointer at `fast-check.yml`, because a list of thirty gates has no reason to live in prose; a table with a sentence a crate does, so this one is held instead. `crate-is-listed` reads `crates/` and the table's **first** cells and fails both ways — a crate the table never names, and a name the table carries that is not a crate. First cells only: the `chart` row's description names `day`, `bhava`, `zodiac` and `foundation`, which are modules, and reading whole rows reports four crates that do not exist. Proved red by renaming one row before it was believed. |
| 2026-09-22 | **The plan is corrected where this work falsified it, and the register has an open question again.** Two pieces of plan integrity rather than code. Phase 5/6's exit criterion asked that *composed text match the baseline engine byte for byte in four languages on the snapshot set*. It was written before the composers were designed and **cannot be met by the design that followed** — which is the criterion's fault and not the design's. A composer emits a **plan**: message keys and slots, holding no words, so one plan renders in every locale the engine carries; the sentences are this SDK's own, written and translated here, and there is no byte of the engine's prose for them to match. Where the engine's own words *are* said — a rule's reading, a graha in a bhava, a nakshatra's phala — they are said **verbatim**, because the migration carries the record across unchanged, and that is the half worth keeping. The clause now reads as the design promises and as `interpret-measured.md` holds on every run: 21 803 items over 93 charts, every one rendering from its own locale's message with no fallback and no warning. And **Q38 is open**: whether a value the SDK computes but does not catalogue should become a kind. `rules::longevity` computes all three ayurdaya methods, the four haranas, fifteen maraka reasons and the dasha-level vulnerability, and serialises them in the very kebab-case the corpus keys by; `LifeClass`, `Quadrant`, the bhava predicates and `Muhurtas`' Abhijit and Brahma are the same shape. **74 of the 126 unmigrated state readings wait on it**, and a kind's number is permanent at the C boundary, so it is the maintainer's. Three options are recorded with their trade-offs and a recommendation — a kind per family as its module lands, because `entity-names.md` §4 refuses a name without a source and the phases that build these modules are the ones that will have the texts open. The resume step that claimed an empty register was true when written and is not now. |
...
| 2026-09-27 | **Gochar crosses to every binding.** `gochar_json` (`{instants, from}`) on the chart request, sections 53 and 54 (fixed, since the request settles the count, and each binding checks the rows divide by the chart count), `chart.gochar` in Node, Python and Dart mirroring the Rust type, parity across all four on every graha of two instants, and an ABI test holding every cell to the façade's reading. Crossing it found a reading that named its reference sign but not what it was counted from: `Reference { from, sign }` says both, `GocharFrom` moved into the crate. The node knobs cross by `of` (`None` for a member the boundary does not know), and the Brahma rule's distinct-code test deduplicated unsorted — it would pass two equal codes apart — and now sorts. Python spells the reference `from_`; its request keeps `"from"` through a functional `TypedDict`. Next: Sade Sati once a source is in hand, the hit list over the crossing search, and the Ashtakavarga's transit scoring. |
