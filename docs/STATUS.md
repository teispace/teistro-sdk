...
- Phase 2's close: a `CROSSINGS` override in the port with its kit
  check, and the heliacal phenomena over the rise and set page; the
  sidereal time expression question (`02-engine-findings.md`, the
  observation not yet filed); cusp speeds and house positions when the
  chart layer needs them (Phase 4). The completion's
