...
**Last updated:** 2026-09-22 (**the catalogue names forty dasha systems
...
2. The bindings' remaining work. Built: the Dart binding from the same
   ...
   **parts** at the boundary with a `MessagePart` in each binding, which
   ...
   the binding.
2a. **A `RashiDefinition` beside `UduDefinition`.** A consumer can
   register a nakshatra-seeded dasha and ask for it by key; a sign-based
   one has no definition to arrive as, so `STHIRA` and `VARNADA` are shut
   to a consumer holding the text exactly as they are to this build
   (`03-design/dasha-coverage-measured.md`). Of the twenty-two systems
   the catalogue names and nothing computes, 3 are supplyable today and
   19 by nobody, and that second number is the SDK's to move rather than
   a source's.
