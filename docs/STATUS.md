...
2b. **Tajika, after the office-bearers** (`03-design/muntha.md`, "The
   ...
   **the caller has founded**, and reproduce the source's worked year end
   to end. Left, in order: the office-bearers' **crossing**, which needs
   the residence knob decided (a place in `varsha_json`, or a function over
   two founded charts); the Panchavargiya bala, whose Drekkana is one
   expression and whose Hudda table is the Egyptian terms with three
   transpositions (C109); and the Tajika aspects. Then the year lord
