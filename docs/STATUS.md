...
**Last updated:** 2026-09-09, end of the fifty-ninth session (the shape
...
2. The local checkout is the repository root; `cargo xtask check-docs`
   ...
   2026-09-05).
3. **The next task is the panchanga blob**, the second half of
   [`chart-at-the-boundary.md`](03-design/chart-at-the-boundary.md), now
   that its shape is measured
   ([`panchanga-at-the-boundary-measured.md`](03-design/panchanga-at-the-boundary-measured.md)).
   The chart half is built: `ts_chart_found` founds a batch, three
   bindings offer `found(one)` and `foundMany(list)` over it, and each
   ships a rectification example. The panchanga blob reuses
   `schemas::day_section` under the same shape name, so a caller that
   renders a chart's day renders a panchanga's with the same code.

   What the measurement settles before a line is written: the per-day
   lists are **ragged**, with an offset column each and one rule for all
   fifteen; a day's three `Option`s cross as a presence flag beside the
   value, never as a sentinel; and the seven `Span<T>` lists are one
   shape, which is what the column-section shaping built in the
   fifty-eighth session is for.

   After it, **`check-parity` over the chart** — it compares 103 values
   today, and the two blobs add 139 distinct paths for 224 in all —
   which is what makes "the three bindings agree" mean something for a
   chart rather than for `positions` alone. Note the cross-phase
   dependency the roadmap now records: **Phase 4 cannot exit before
   Phase 3 delivers the completion's centre step**, because all 55
   recorded charts are topocentric and the examples had to change
   profile to run.
