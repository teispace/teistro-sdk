...
3. **The next task is the chart document's round trip and its number
   ...
   - **nothing in the layer reads back**: 65 types derive `Serialize`,
     ...
     unwritable;
   - **the canonical form was not a fixed point** once a document
     carried an instant — the grammar's twelve decimals were three past
     what an `f64` resolves at a Julian day's magnitude. **Fixed**: the
     form now writes the shortest decimal that reads back as the same
     double, every content hash moved once, and no computed value did.
