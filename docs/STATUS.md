...
3. **Phase 3, the built-in ephemeris — milestone M3 — is all but
   ...
   **The first four of those are built.** The adapters export a plugin
   ...
   functions into what is callable today (**138**: 62 before plain
   structs were learned, 95 before arrays, 112 before strings and pointers
   inside structs, 135 before outputs sized by another call), what the
   adapter will never hand over whatever its shape (12, each of which
   opens, closes or rebinds the context every chart is cast on), and what
   is queued behind one more shape (11). Strings closed the way the page predicted they
