...
- Phase 2's close: the accuracy document the roadmap's exit names
  (every `astro` row's measured agreement with Teimeris in one generated
  table), a `CROSSINGS` override in the port with its kit check, and the
  heliacal phenomena over the rise and set page; cusp speeds and house
  positions when the chart layer needs them (Phase 4). The completion's
