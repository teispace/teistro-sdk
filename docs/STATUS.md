...
- Phase 1's remaining deliverables, in the roadmap's order: the `intl`
  ...
  check-dart` analysing, format-checking and running fifteen tests against
  the real library). Next in it: the Dart binding's host-implemented
  provider, the two bindings' packaging, and the parity gate; then the
  test-only infrastructure, the hash matrix, the conformance repository
  (ADR-0022), the docs site. The completion's
