...
- Phase 1's remaining deliverables, in the roadmap's order: the `intl`
  ...
  and the twelve-hour clock with the day periods (thirty-fifth session);
  still to come in it: the composite provider's precedence with the
  bindings, day-period ranges per locale and the `zone` option,
  transliteration, XLIFF, rich renderers per binding.
