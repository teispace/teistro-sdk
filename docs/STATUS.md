...
1. Phase 2's astronomy as above (it is "Now"): a chart founded on a
   ...
   `classical-chart-measured.md`). Its zodiac and its nine grahas are the
   text's (2026-09-26); left, in the page's order of work, are the day
   from the text's own sunrise, the text's Lagna through a new `angles`
   port method (vtable ABI 5) with the house systems built from it, the
   `deviation` stamp, and an ephemeris key so every binding reaches the
   text by name. The settable atmosphere for the rise and set solver
