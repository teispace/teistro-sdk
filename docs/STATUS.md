...
2b. **Tajika, after the office-bearers** (`03-design/muntha.md`, "The
   ...
   **annual dashas** are built in Rust (2026-09-24,
   ...
   balance (C123) as readings. Left: they **cross** the boundary, through
   `varsha_json` and a section of periods, into the three bindings. The
   "four" this item used to promise were three: Charak's chapter V names
   those three, and Varsha Narayana is in no book read.
2c. **The shared examples, printed alike.** Measured 2026-09-22 by running
   all eleven examples every binding carries: **4 print identical output**
   in Node, Python and Dart (annual chart, calendar, chart reading,
   panchanga) and **7 differ**, every difference presentation rather than
   value — a full catalogue key against a short one, Python's `False`
   against `false`, `2200000.0` against `2200000`, an error's class name.
   The parity gate already holds the values; nothing holds the examples'
   shape to one another, so a claim that they match was a sentence and was
   wrong once. Normalise the seven, then gate all eleven in `check-parity`.
