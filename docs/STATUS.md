...
**Last updated:** 2026-09-06, end of the thirty-third session (the Node
...
- Phase 1's remaining deliverables, in the roadmap's order: the `intl`
  ...
  running fourteen tests). Next in it: a host-implemented ephemeris
  provider from JavaScript, the Dart binding the same way, the two
...
2. The bindings' remaining work: a host-implemented ephemeris provider
   from JavaScript (the port's vtable filled by a `ThreadsafeFunction` or
   a synchronous callback, as spike 2 measured at 0.47 µs a call), the
