...
2. The local checkout is the repository root; `cargo xtask check-docs`
   ...
   2026-09-05).
3. **The next task is the chart foundation and the panchanga day at the
   boundary**, designed in
   [`chart-at-the-boundary.md`](03-design/chart-at-the-boundary.md)
   and ready to build. It is **Phase 4's own exit condition** — the
   golden vectors reproduced in three bindings — and nothing above
   `positions` crosses today, which is why each binding's examples stop
   where they do and why `check-parity` compares 103 values rather than
   a chart.
