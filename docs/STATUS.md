...
1. Phase 2's astronomy as above (it is "Now"): a chart founded on a
   ...
   stamp and `sdk.chart().angles`. Left: the C selector's key, so Node,
   Dart, Python and wasm reach the text by name, and the parity gate
   comparing a classical chart. The settable atmosphere for the rise and set solver
