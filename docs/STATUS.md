...
- Phase 2's astronomy, continued: a `CROSSINGS` override in the port
  with its kit check; cusp speeds and house positions when the chart
  layer needs them; the equation of time and the planetary phenomena
  (`13-astronomy-layer.md` rows). The completion's centre, corrections
  and equinox steps wait for the built-in ephemeris (Phase 3). The
  memo's R3 stays open (a third source; the committee's earlier years
  are not online).
