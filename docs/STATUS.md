...
**Last updated:** 2026-09-06, end of the thirty-second session (the Node
...
- Phase 1's remaining deliverables, in the roadmap's order: the `intl`
  ...
  tables into `sdk.entity`, `hi-Deva-IN` and `sa-Deva` created); still
  to come in it: the composite provider's precedence with the bindings,
  ...
  Next in it: the napi addon generated from the description and the
  ergonomic layer over it, then the Dart binding the same way, the two
  bindings' packaging, and the parity gate; then the test-only
  infrastructure, the hash matrix, the conformance repository (ADR-0022),
  the docs site. The completion's
...
2. The Node addon and the Dart binding over the description: the napi
   emitter from the spike's `gen_node.rs` and the Dart emitter from its
   `gen_dart.rs`, rewritten over `teistro-idl` (roles, finaliser-backed
   handles with `dispose`, the host provider wrapped into the port's
   vtable), the hand-written ergonomic layers with the branded types and
   the typed intl accessors, packaging with the `buildinfo` handshake,
   and the parity gate comparing the generated surfaces with the
   description and the two bindings' canonical JSON with each other. The
   TypeScript surface and the blob decoders exist since the thirty-second
