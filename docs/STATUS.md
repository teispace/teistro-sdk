...
4. What is built, and what runs it. **Eight crates were missing from this
   ...
   | `tajika` | the annual chart: the Sun's return to where it stood at birth, in the chart's own zodiac, with the tropical and mean readings named as the rivals they are; and the **Muntha**, the birth lagna progressed one sign per completed year, with its lord |
...
2b. **Tajika, after the Muntha** (`03-design/muntha.md`, "The order of
   work"). The Muntha and its lord are built and cross every binding; the
   lord of the year needs three things this build lacks, in this order:
   the other four office-bearers over an annual chart **the caller has
   founded** (three of them need its place, so they cannot ride with the
   instant as the Muntha does), with the Tri-Rashi lords derived from the
   Dorothean triplicities (C108); the Panchavargiya bala, whose Drekkana is
   one expression and whose Hudda table is the Egyptian terms with three
