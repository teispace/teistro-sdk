...
| horizon convention | what a rise or set is reckoned for: which point of the disc (centre, upper limb, lower limb), whether refraction lifts it, and the altitude the point reaches at the event; the settings' sunrise knob names three and a custom altitude (`crates/port-ephemeris`) |
| override policy | which implementation of a computation both the SDK and a provider offer is used: the provider's declared one (`PREFER_NATIVE`), the SDK's everywhere (`SDK_ONLY`), or the provider's or a refusal (`NATIVE_ONLY`); ADR-0013 |
