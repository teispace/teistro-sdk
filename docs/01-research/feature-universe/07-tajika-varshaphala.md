...
| Mudda (Varsha Vimshottari), Varsha Yogini and Patyayini dashas — **read and built** (2026-09-24, [`annual-dashas.md`](../../03-design/annual-dashas.md)): Charak ch. V gives all three with worked years, the *Tajika Nilakanthi* the Patyayini's tie rule and antardasha order and the Mudda's balance at the return | annual chart | the clock a year's units run on (C122); where the balance comes from (C123); Kalidasa's Mudda not built | partial | all | P0 |
