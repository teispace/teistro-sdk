...
- **The areas the remaining phases add.** Dashas, strengths, rules and
  ...
  The rules operations settled it without an area of their own
  (`03-design/rules-at-the-boundary.md`, 2026-09-17): a rule reads a founded
  chart and the boundary keeps no documents between calls, so rules ride on
  `chart.found` as a `rules` option, answered in the same crossing, in every
  binding.
