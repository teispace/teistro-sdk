...
when `crates/calendar` is built and again when the source memo
(`docs/calendars/bikram-sambat.md`, the research standard of
`09-guidelines/04-adding-a-calendar.md`) is reviewed. Derives from
...
pub enum MonthStartRule {
    ...
    NextDayAfter { threshold: DayFraction }, // or the next day when the sankranti falls after the threshold
...
`astro` under the stamped frame, convert each to a civil day at the
stamped place and day boundary, apply the `MonthStartRule`, and take
the differences as month lengths; the first Baisakh of the year is the
year's start. The rule is chosen by measurement: each candidate is run
over the table's own span and the one that reproduces the most official
month lengths is the default, its miss rate and the divergent dates
published. The extension is generated by `cargo xtask gen calendars`
...
1. The `MonthStartRule` the official almanac follows, and the frame it
   computes in (which ayanamsha or siddhanta): the memo answers both
   from the authority's own publications before the extension is
   regenerated; until then the baseline engine's computed table ships
   as rank-2 data with its stamp.
