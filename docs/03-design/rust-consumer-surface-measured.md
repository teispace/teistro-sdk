...
|---|---|---|
| an area's operations come from one SDK crate, so a Rust consumer already has the area | falsified | 7 of 8 disagree; more than one: `almanac (10)`, `calendar (2)`, `chart (11)`, `engine (2)`, `frame (2)`, `intl (3)`, `time (4)` |
...
| every type an area's signature names is reachable from the crate root | falsified | 4 of 44 disagree; not reachable: `Content`, `Dasha`, `Labels`, `Theme` |
...
| `ts_chart_layout_row` | `chart` | 1 | `teistro-core` |
...
| `ts_error_free` | `context` | 1 | `teistro-core` |
