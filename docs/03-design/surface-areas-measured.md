...
|---|---|---|
| every boundary module is an area a consumer sees | falsified | 5 of 14 disagree; never reached: `blob`, `context`, `lib`, `provider`, `strings` |
...
| an entry point's name already carries its own module | falsified | 10 of 46 disagree; the exceptions are `ts_abi_version`, `ts_sdk_version`, `ts_catalogue_version`, `ts_default_profile`, `ts_build_info`, `ts_status_message`, `ts_string_free`, `ts_key_parse`, `ts_key_name`, `ts_context_new_with_provider` |
...
The remaining 4 are **inconsistencies rather than exceptions**, and each is a line to fix while the surface is being restructured anyway: `ts_key_parse`, `ts_key_name`, `ts_context_new_with_provider`, `ts_string_free`.
...
| `keyId` | `ts_key_parse` | `keys` |
| `keyName` | `ts_key_name` | `keys` |
| `dispose` | — |  |
