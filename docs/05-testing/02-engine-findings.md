...
| D3 | The context's error record is not written by every failure: 255 `return TM_ERR_…;` statements bypass `tm_fail`, 123 of them null-check prologues of the form `if (!ctx \|\| !x) return TM_ERR_INVALID_ARG;`. The header promises the record "stays valid until the next call"; after such a failure it still holds the previous failure | reproduced in C: `tm_star_find(ctx, "no such star")` records −7 and *no star named 'no such star'*; `tm_local_to_utc(ctx, NULL, …)` then returns −1 and the record still says −7 and the star's message. The engine's generators surface every struct input as nullable, so a binding reaches this path, and one that reads the record after a failure reports the wrong mistake | **open**: the fix is mechanical — split each prologue into `if (!ctx) return …;` and a `tm_fail` that names the null pointer, same status — but it touches 123 sites across the core and wants its own change with the engine's full suite | the passthrough snapshots the record before a call that has a context and attaches its message only when the record **changed** across the call; otherwise the refusal says the engine recorded no message (`tests/passthrough.rs`, `an_earlier_failure_s_message_is_not_repeated_as_this_one_s`) |

D1's and D2's commits are in the engine's history and not yet on its remote; the
