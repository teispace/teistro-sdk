...
14. Bhava chalit. Every chart's `bhava_chalit` calls itself `equal-house`
    and is Vehlow: equal houses *centred* on the ascendant, where equal
    houses *start* at it — fifteen degrees apart. Measured over all 55
    charts, Vehlow reproduces every recorded placement on 55 of them,
    Sripati on 20 and Porphyry on none
    (`03-design/chart-bhava-chalit.md`, generated by `cargo xtask chalit`).
