...
| the Indian lunisolar calendar | **designed**: `03-design/calendar-indian-lunisolar.md`, written from a measurement over 12 368 lunar months (`cargo xtask lunisolar`, held by `check-lunisolar`). The rule is the count of sankrantis in the month — none adhika, one ordinary, two kshaya — and it reproduces the corpus's marking on all fifty-five days. It computes from the Surya Siddhanta's own Sun and Moon, so it takes no ephemeris, and an adhika month needs **no** naming rule of its own, so `panchanga` gains the mark and changes nothing else. The page is deliberately about the mark and **not** dates: a lunisolar date's day repeats one day in forty-four and is skipped one in twenty-six, so it needs two flags `CalendarDate` does not carry, and the full `CalendarSystem` waits until a consumer wants one |
