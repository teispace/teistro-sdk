...
| `chart` | birth data, foundation (positions in both frames with speeds, declinations, RA; cusps; frames), chart kinds. Built so far: `day` (the day an instant belongs to, which is not its civil date) and `bhava` (the twelve bhavas with their madhya as well as their sandhi, and a placement that carries the chalit that made it), both against all 55 recorded charts (`03-design/chart-foundation.md`) | `astro`, `time`, `calendar`, `core` | P0 | `ChartFoundation` |
