...
pub(crate) fn call(
    ...
    match function {
        ...
        "tm_houses_calc_many" => {
            let reqs: Vec<sys::tm_houses_request> = objects(args, "reqs", read_tm_houses_request)?;
            // SAFETY: `tm_house_cusp_count` takes values only and answers a length.
            let mut cusps: Vec<f64> = room(extent(&[reqs.iter().map(|one| unsafe { sys::tm_house_cusp_count(one.system) }).max().unwrap_or(0), reqs.len()], "cusps")?, "cusps")?;
            let mut out_angles: Vec<sys::tm_angles> = room(reqs.len(), "out_angles")?;
            let before = record(context);
            // SAFETY: the context is the adapter's own and live for this
            // call; every input array is a `Vec` the arm owns, passed with
            // its own length; every output array is a `Vec` the arm sized
            // to the engine's stated extent, passed with its own length.
            let answered = unsafe { sys::tm_houses_calc_many(context, reqs.as_ptr(), reqs.len(), cusps.as_mut_ptr(), cusps.len(), out_angles.as_mut_ptr(), out_angles.len()) };
            checked(context, &before, answered, "tm_houses_calc_many")?;
            Ok(json!({"cusps": cusps, "out_angles": out_angles.iter().map(write_tm_angles).collect::<Vec<_>>()}))
        }
        "tm_house_position" => {
