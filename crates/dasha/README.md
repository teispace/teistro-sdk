...
| [`rashi`](src/rashi.rs) | a sign-based system as a row — its start, order, length and named lord — over the chart it reads, with footedness and parity as distinct types; the eight shipped rows; `stronger_sign`, BPHS ch. 46's sign strength, and `RashiRules`, its dual-lord and start readings beside the recording engine's |
