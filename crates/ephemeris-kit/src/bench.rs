//! One timing helper for every measurement of the kit: the median of the
//! best of `rounds` rounds, in microseconds, with the 90th percentile, the
//! same statistic the binding harnesses report.
...
mod tests {
    ...
    #[test]
    fn a_measurement_keeps_its_counts_and_renders() {
        let mut calls = 0u32;
        let row = bench("nothing", 5, 2, 2, || calls += 1);
        assert_eq!(calls, 2 * (2 + 5));
        assert_eq!((row.iterations, row.rounds), (5, 2));
        assert!(row.median_us >= 0.0 && row.p90_us >= row.median_us);
        assert!(markdown(&[row]).contains("| nothing |"));
    }
}
