...
pub(crate) fn doc(text: &str, indent: &str) -> String {
    line_comment(text, indent, "/// ")
}

/// A doc as a run of `///` lines, for a text that may hold more than
/// one.
///
/// Three writers here used to interpolate a section's `doc` into a
/// single `/// {}` and produced a file Dart could not parse the moment
/// one of them carried a newline: the second line came out bare. The
/// shared `line_comment` always handled it; these three did not call it.
fn doc_lines(text: &str) -> String {
    doc(text, "").trim_end().to_owned()
}

pub(crate) fn preamble(api: &Api, ignores: &str) -> String {
    format!(
