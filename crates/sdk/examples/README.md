...
| [`interpretation.rs`](interpretation.rs) | the same birth record said in English and in Nepali | a composer returns a **plan**, not prose: an ordered list of message keys and slots that holds no words, so one plan renders in every locale and the consumer may reorder or drop items first. Two composers concatenate into one plan, which is what a flat plan is for. `Rendered` says which locale answered, so a fallback cannot pass unnoticed; and what a composer cannot say — the lagna, which these messages have no word for — it does not guess |
...
There is a further file, [`parity.rs`](parity.rs), and it is not one of
