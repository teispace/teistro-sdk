...
//! the same hold of the lock.

use std::path::{Path, PathBuf};
use std::sync::{Mutex, PoisonError};

use teimeris::{
    Body as TmBody, Columns, Context, ErrorKind, Flags, Observer as TmObserver, Profile,
    TimeScale as TmScale,
};
use teistro_spike_port::model::{
    AyanamshaId, Body, Capabilities, Cell, CellStatus, Center, Coordinates, EphemerisKind, Equinox,
    Frame, Identity, Obliquity, Overrides, PositionColumns, PositionRequest, ProviderError, Source,
    TimeScale, Zodiac,
};
...
fn map_status(code: i32) -> CellStatus {
    match teimeris::Status(code) {
        ...
        _ => CellStatus::Provider(CODE_BASE + code),
    }
}
...
fn map_flags(frame: Frame, speeds: bool) -> Flags {
    ...
    let switches = [
        ...
        (frame.center == Center::Topocentric, Flags::TOPOCENTRIC),
        (frame.center == Center::Heliocentric, Flags::HELIOCENTRIC),
        (frame.center == Center::Barycentric, Flags::BARYCENTRIC),
        (matches!(frame.zodiac, Zodiac::Sidereal(_)), Flags::SIDEREAL),
    ];
...
impl EphemerisProvider for TeimerisProvider {
    ...
    fn positions(&self, request: &PositionRequest<'_>) -> Result<PositionColumns, ProviderError> {
        ...
        let observer = request
            .observer
            .map(|o| TmObserver::new(o.longitude_deg, o.latitude_deg, o.altitude_m));
        ...
        let raw = self.with_context(|ctx| {
            if let Zodiac::Sidereal(id) = request.frame.zodiac {
                ctx.set_ayanamsha(i32::from(id.0), 0.0, 0.0)
                    .map_err(|error| map_error(&error, f64::NAN))?;
            }
