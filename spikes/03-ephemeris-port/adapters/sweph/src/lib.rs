...
use teistro_spike_port::model::{
    AyanamshaId, Body, Capabilities, Cell, CellStatus, Center, Coordinates, EphemerisKind, Equinox,
    Frame, Identity, Obliquity, Observer, Overrides, PositionColumns, PositionRequest,
    ProviderError, Source, TimeScale, Zodiac,
};
use teistro_spike_port::provider::{EphemerisProvider, validate};
use teistro_spike_port::sefile;
...
impl Session<'_> {
    ...
    #[allow(unsafe_code, reason = "a C entry point; see the safety comment")]
    fn set_topo(&self, observer: Observer) {
    ...
    #[allow(unsafe_code, reason = "a C entry point; see the safety comment")]
    fn set_sid_mode(&self, id: AyanamshaId) {
        // SAFETY: three scalars into process-wide state, under the lock.
        unsafe { ffi::swe_set_sid_mode(i32::from(id.0), 0.0, 0.0) };
    }
...
fn map_flags(frame: Frame, speeds: bool) -> i32 {
    let switches = [
        ...
        (frame.center == Center::Topocentric, ffi::flag::TOPOCTR),
        (frame.center == Center::Heliocentric, ffi::flag::HELCTR),
        (frame.center == Center::Barycentric, ffi::flag::BARYCTR),
        (
            matches!(frame.zodiac, Zodiac::Sidereal(_)),
            ffi::flag::SIDEREAL,
        ),
    ];
    switches
...
fn map_cell(answer: &Answer) -> Cell {
    if answer.retflag < 0 {
        return Cell {
            status: CellStatus::Provider(CALC_FAILED),
            ..Cell::EMPTY
        };
    }
...
impl EphemerisProvider for SwephProvider {
    ...
    fn positions(&self, request: &PositionRequest<'_>) -> Result<PositionColumns, ProviderError> {
        ...
        if let Zodiac::Sidereal(id) = request.frame.zodiac {
            session.set_sid_mode(id);
        }
    ...
    fn ayanamsha_deg(
        ...
        if !self.capabilities.ayanamshas.contains(&id) {
            return Err(ProviderError::unsupported("this ayanamsha"));
        }
        let session = Session::take();
        session.set_sid_mode(id);
...
mod tests {
    ...
    fn threads_interleaving_sidereal_modes_and_observers_get_the_serial_bits() {
        ...
        let cases = [
            ...
            (
                Frame::CANONICAL.with_zodiac(Zodiac::Sidereal(AyanamshaId::LAHIRI)),
                None,
            ),
            (
                Frame::CANONICAL.with_zodiac(Zodiac::Sidereal(AyanamshaId::RAMAN)),
                None,
            ),
            (topocentric, Some(kathmandu)),
            (topocentric, Some(zurich)),
        ];
        let compute = |(frame, observer): (Frame, Option<Observer>)| {
