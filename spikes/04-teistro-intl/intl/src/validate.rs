...
fn check_references(
    ...
    for entity in &sig.entities {
        if !entity_keys.contains(entity.as_str()) {
            findings.error(
                tag,
                key,
                format!("`:entity` names `{entity}`, which is not in the entity namespace"),
            );
        }
    }
    for kind in sig.params.values().filter_map(|p| match p {
        ...
        if !kinds.contains(kind.as_str()) {
            findings.error(
                tag,
                key,
                format!("`kind={kind}` is not a group of the entity namespace"),
            );
        }
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::source::{Entity, Namespace, spike_root};
    ...
    fn each_gate_fires() {
        ...
        set(
            ...
            "sdk.reason",
            "dangling",
            "{sdk.reason.nowhere :msg} {graha.PLUTO :entity} {$x :entity kind=asteroid}",
        );
        set(
            &mut t,
            "en-Latn",
        ...
        for expected in [
            ...
            "dangling: `:entity` names `graha.PLUTO`",
            "dangling: `kind=asteroid` is not a group",
