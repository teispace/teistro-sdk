...
/// The bytes of a blob the library filled, copied into a byte array and the
/// blob freed, so nothing of the library's outlives the call.
fn take_blob(blob: &mut ffi::blob::TsBlob) -> Vec<u8> {
...
fn refused(raw: &mut ffi::context::TsError) -> Error {
...
impl Context {
    ...
    pub fn new(
        options: JsValue,
        provider: JsValue,
        provider_positions: Option<js_sys::Function>,
    ) -> Result<Self> {
        let options: Option<ContextOptions> = from_js(options)?;
        ...
        let provider: Option<crate::provider::ProviderInfo> = from_js(provider)?;
    ...
    #[wasm_bindgen(js_name = settingsHash)]
    pub fn settings_hash(&self) -> Result<JsValue> {
        // SAFETY: every field is a plain integer, float or pointer, so
        // all-zero is a valid value; a size, where the struct has one, is set
        // before the call reads it.
        ...
        self.leave()?;
        self.check(status)?;
        Ok(to_js(&unsafe { Hash::write(&out_hash) })?)
    }
    ...
    #[wasm_bindgen(js_name = calendarFromFixed)]
    pub fn calendar_from_fixed(&self, calendar: String, fixed: f64) -> Result<JsValue> {
        ...
        self.leave()?;
        self.check(status)?;
        Ok(to_js(&unsafe { CalendarDate::write(&out_date) })?)
    }

    /// The fixed day of a date, validated: a date the calendar does not have
    /// is `INVALID_ARG` with the `NONEXISTENT_DATE` detail.
    #[wasm_bindgen(js_name = calendarToFixed)]
    pub fn calendar_to_fixed(&self, date: JsValue) -> Result<f64> {
        let date: CalendarDate = from_js(date)?;
    ...
    #[wasm_bindgen(js_name = calendarConvert)]
    pub fn calendar_convert(&self, date: JsValue, into: String) -> Result<JsValue> {
        let date: CalendarDate = from_js(date)?;
        let held_date = date.read()?;
        let raw_date = held_date.as_c();
        let date = &raw const raw_date;
    ...
    #[wasm_bindgen(js_name = calendarMonthLength)]
    ...
    #[wasm_bindgen(js_name = calendarIsLeap)]
    ...
    #[wasm_bindgen(js_name = calendarWeekday)]
    pub fn calendar_weekday(&self, date: JsValue) -> Result<u32> {
        let date: CalendarDate = from_js(date)?;
        let held_date = date.read()?;
        let raw_date = held_date.as_c();
        let date = &raw const raw_date;
    ...
    #[wasm_bindgen(js_name = chartLayoutRow)]
    pub fn chart_layout_row(&self, key: String) -> Result<String> {
        let key = std::ffi::CString::new(key).map_err(|e| Error::from_reason(e.to_string()))?;
        let mut out_json = ffi::string::TsString::empty();
        self.enter();
        // SAFETY: the handle is live and every pointer is valid for the call.
    ...
    #[wasm_bindgen(js_name = chartFound)]
    pub fn chart_found(&self, request: JsValue) -> Result<Vec<u8>> {
        let request: ChartRequest = from_js(request)?;
    ...
    #[wasm_bindgen(js_name = timeResolve)]
    pub fn time_resolve(&self, civil: JsValue, zone: JsValue) -> Result<JsValue> {
        let civil: CivilDateTime = from_js(civil)?;
        let held_civil = civil.read()?;
        let raw_civil = held_civil.as_c();
        let civil = &raw const raw_civil;
        let zone: ZoneSpec = from_js(zone)?;
        ...
        self.leave()?;
        self.check(status)?;
        Ok(to_js(&unsafe { ZoneResolution::write(&out_resolution) })?)
    }

    /// The civil date-time of a UTC instant in a zone, in a calendar, with
    /// the resolution that applied.
    #[wasm_bindgen(js_name = timeCivil)]
    pub fn time_civil(&self, jd_utc: f64, zone: JsValue, calendar: String) -> Result<JsValue> {
        let zone: ZoneSpec = from_js(zone)?;
        let held_zone = zone.read()?;
        let raw_zone = held_zone.as_c();
        let zone = &raw const raw_zone;
    ...
    #[wasm_bindgen(js_name = timeConvert)]
    pub fn time_convert(&self, jd: f64, from: String, to: String) -> Result<JsValue> {
        ...
        self.leave()?;
        self.check(status)?;
        Ok(to_js(&unsafe { TimeConversion::write(&out_conversion) })?)
    }
    ...
    #[wasm_bindgen(js_name = timeDeltaT)]
    pub fn time_delta_t(&self, jd_ut1: f64) -> Result<JsValue> {
        // SAFETY: every field is a plain integer, float or pointer, so
        // all-zero is a valid value; a size, where the struct has one, is set
        // before the call reads it.
        ...
        self.leave()?;
        self.check(status)?;
        Ok(to_js(&unsafe { DeltaT::write(&out_delta_t) })?)
    }
    ...
    #[wasm_bindgen(js_name = intlLoadPack)]
    pub fn intl_load_pack(&self, bytes: Vec<u8>) -> Result<JsValue> {
        // SAFETY: every field is a plain integer, float or pointer, so
        // all-zero is a valid value; a size, where the struct has one, is set
        // before the call reads it.
    ...
    /// context.
    #[wasm_bindgen(js_name = intlLocale)]
    ...
    #[wasm_bindgen(js_name = intlTransliterate)]
    pub fn intl_transliterate(&self, text: String, from: String, to: String) -> Result<String> {
    ...
    #[wasm_bindgen(js_name = intlEntity)]
    pub fn intl_entity(&self, key: String) -> Result<String> {
        let key = std::ffi::CString::new(key).map_err(|e| Error::from_reason(e.to_string()))?;
    ...
    #[wasm_bindgen(js_name = intlRender)]
    pub fn intl_render(&self, key: String, params_json: Option<String>) -> Result<Vec<u8>> {
        let key = std::ffi::CString::new(key).map_err(|e| Error::from_reason(e.to_string()))?;
    ...
    pub fn positions(&self, request: JsValue) -> Result<Vec<u8>> {
        let request: PositionRequest = from_js(request)?;
        let held_request = request.read()?;
        let raw_request = held_request.as_c();
        let request = &raw const raw_request;
        let mut out_blob = ffi::blob::TsBlob::empty();
    ...
    #[wasm_bindgen(js_name = panchangaDays)]
    pub fn panchanga_days(&self, request: JsValue) -> Result<Vec<u8>> {
        let request: PanchangaRequest = from_js(request)?;
        let held_request = request.read()?;
        let raw_request = held_request.as_c();
        let request = &raw const raw_request;
        let mut out_blob = ffi::blob::TsBlob::empty();
    ...
    #[wasm_bindgen(js_name = ephemerisManifest)]
    pub fn ephemeris_manifest(&self) -> Result<String> {
        let mut out_json = ffi::string::TsString::empty();
        self.enter();
        // SAFETY: the handle is live and every pointer is valid for the call.
        let status =
    ...
    #[wasm_bindgen(js_name = ephemerisCall)]
    pub fn ephemeris_call(&self, function: String, arguments_json: String) -> Result<String> {
        let function =
...
#[wasm_bindgen(js_name = catalogueVersion)]
...
#[wasm_bindgen(js_name = frameCanonical)]
pub fn frame_canonical() -> Result<JsValue> {
    // SAFETY: every field is a plain integer, float or pointer, so
    // all-zero is a valid value; a size, where the struct has one, is set
    // before the call reads it.
    ...
    Ok(to_js(&unsafe { Frame::write(&out_frame) })?)
}

/// Packs a frame into the bits a position request carries.
#[wasm_bindgen(js_name = framePack)]
pub fn frame_pack(frame: JsValue) -> Result<u32> {
    let frame: Frame = from_js(frame)?;
...
/// `INVALID_ARG`.
#[wasm_bindgen(js_name = frameUnpack)]
pub fn frame_unpack(bits: u32) -> Result<JsValue> {
    // SAFETY: every field is a plain integer, float or pointer, so
    // all-zero is a valid value; a size, where the struct has one, is set
    // before the call reads it.
    let mut out_frame: ffi::frame::TsFrame = unsafe { core::mem::zeroed() };
...
#[wasm_bindgen(js_name = calendarJdOfFixed)]
pub fn calendar_jd_of_fixed(fixed: f64) -> Result<f64> {
    // SAFETY: every pointer is valid for the call.
...
#[wasm_bindgen(js_name = calendarFixedOfJd)]
pub fn calendar_fixed_of_jd(jd: f64) -> Result<JsValue> {
    ...
    let handed = CalendarFixedOfJdResult {
        value: value as _,
