// Generated by teistro-intl from i18n/en-Latn. Do not edit.
// Keys and parameter shapes only; text comes from packs.

export type Gender = 'm' | 'f' | 'n';
export type AvasthaBaladiKey = 'avastha_baladi.BALA' | 'avastha_baladi.KUMARA' | 'avastha_baladi.YUVA' | 'avastha_baladi.VRIDDHA' | 'avastha_baladi.MRITA';
export type AvasthaDeeptadiKey = 'avastha_deeptadi.DEEPTA' | 'avastha_deeptadi.SWASTHA' | 'avastha_deeptadi.MUDITA' | 'avastha_deeptadi.SHANTA' | 'avastha_deeptadi.DINA' | 'avastha_deeptadi.DUKHI' | 'avastha_deeptadi.VIKALA' | 'avastha_deeptadi.KHALA' | 'avastha_deeptadi.KOPA';
export type AvasthaJagradadiKey = 'avastha_jagradadi.JAGRAT' | 'avastha_jagradadi.SWAPNA' | 'avastha_jagradadi.SUSHUPTI';
export type AvasthaLajjitadiKey = 'avastha_lajjitadi.LAJJITA' | 'avastha_lajjitadi.GARVITA' | 'avastha_lajjitadi.KSHUDHA' | 'avastha_lajjitadi.TRISHITA' | 'avastha_lajjitadi.MUDITA' | 'avastha_lajjitadi.KSHOBHITA';
export type AyanaKey = 'ayana.UTTARAYANA' | 'ayana.DAKSHINAYANA';
export type AyanamshaKey = 'ayanamsha.FAGAN_BRADLEY' | 'ayanamsha.LAHIRI' | 'ayanamsha.DELUCE' | 'ayanamsha.RAMAN' | 'ayanamsha.USHASHASHI' | 'ayanamsha.KRISHNAMURTI' | 'ayanamsha.DJWHAL_KHUL' | 'ayanamsha.YUKTESHWAR' | 'ayanamsha.JN_BHASIN' | 'ayanamsha.BABYL_KUGLER1' | 'ayanamsha.BABYL_KUGLER2' | 'ayanamsha.BABYL_KUGLER3' | 'ayanamsha.BABYL_HUBER' | 'ayanamsha.BABYL_ETPSC' | 'ayanamsha.ALDEBARAN_15TAU' | 'ayanamsha.HIPPARCHOS' | 'ayanamsha.SASSANIAN' | 'ayanamsha.GALCENT_0SAG' | 'ayanamsha.J2000' | 'ayanamsha.J1900' | 'ayanamsha.B1950' | 'ayanamsha.SURYASIDDHANTA' | 'ayanamsha.SURYASIDDHANTA_MSUN' | 'ayanamsha.ARYABHATA' | 'ayanamsha.ARYABHATA_MSUN' | 'ayanamsha.SS_REVATI' | 'ayanamsha.SS_CITRA' | 'ayanamsha.TRUE_CHITRA' | 'ayanamsha.TRUE_REVATI' | 'ayanamsha.TRUE_PUSHYA' | 'ayanamsha.GALCENT_RGILBRAND' | 'ayanamsha.GALEQU_IAU1958' | 'ayanamsha.GALEQU_TRUE' | 'ayanamsha.GALEQU_MULA' | 'ayanamsha.GALALIGN_MARDYKS' | 'ayanamsha.TRUE_MULA' | 'ayanamsha.GALCENT_MULA_WILHELM' | 'ayanamsha.ARYABHATA_522' | 'ayanamsha.BABYL_BRITTON' | 'ayanamsha.TRUE_SHEORAN' | 'ayanamsha.GALCENT_COCHRANE' | 'ayanamsha.GALEQU_FIORENZA' | 'ayanamsha.VALENS_MOON' | 'ayanamsha.LAHIRI_1940' | 'ayanamsha.LAHIRI_VP285' | 'ayanamsha.KRISHNAMURTI_VP291' | 'ayanamsha.LAHIRI_ICRC';
export type CharaKarakaKey = 'chara_karaka.ATMAKARAKA' | 'chara_karaka.AMATYAKARAKA' | 'chara_karaka.BHRATRIKARAKA' | 'chara_karaka.MATRIKARAKA' | 'chara_karaka.PUTRAKARAKA' | 'chara_karaka.GNATIKARAKA' | 'chara_karaka.DARAKARAKA' | 'chara_karaka.PITRIKARAKA';
export type ChoghadiyaKey = 'choghadiya.UDVEG' | 'choghadiya.CHAR' | 'choghadiya.LAABH' | 'choghadiya.AMRIT' | 'choghadiya.KAAL' | 'choghadiya.SHUBHA' | 'choghadiya.ROG';
export type DashaSystemKey = 'dasha_system.VIMSHOTTARI' | 'dasha_system.ASHTOTTARI' | 'dasha_system.DWADASHOTTARI' | 'dasha_system.PANCHOTTARI' | 'dasha_system.SHATABDIKA' | 'dasha_system.CHATURASHITI_SAMA' | 'dasha_system.DWISAPTATI_SAMA' | 'dasha_system.YOGINI' | 'dasha_system.TRIBHAGI' | 'dasha_system.CHARA' | 'dasha_system.NARAYANA' | 'dasha_system.PADANADHAMSA' | 'dasha_system.TRIKONA' | 'dasha_system.DRIG' | 'dasha_system.SHOOLA' | 'dasha_system.NIRYANA_SHOOLA' | 'dasha_system.MANDOOKA' | 'dasha_system.KALACHAKRA';
export type DeityKey = 'deity.ASHWINI_KUMARA' | 'deity.YAMA' | 'deity.AGNI' | 'deity.BRAHMA' | 'deity.SOMA' | 'deity.RUDRA' | 'deity.ADITI' | 'deity.BRIHASPATI' | 'deity.NAGA' | 'deity.PITRU' | 'deity.BHAGA' | 'deity.ARYAMAN' | 'deity.SAVITR' | 'deity.VISHVAKARMA' | 'deity.VAYU' | 'deity.INDRAGNI' | 'deity.MITRA' | 'deity.INDRA' | 'deity.NIRRITI' | 'deity.APAS' | 'deity.VISHVADEVA' | 'deity.VISHNU' | 'deity.VASU' | 'deity.VARUNA' | 'deity.AJA_EKAPADA' | 'deity.AHIRBUDHNYA' | 'deity.PUSHAN';
export type DignityKey = 'dignity.DEEP_EXALTED' | 'dignity.EXALTED' | 'dignity.MOOLTRIKONA' | 'dignity.OWN_SIGN' | 'dignity.GREAT_FRIEND' | 'dignity.FRIEND' | 'dignity.NEUTRAL' | 'dignity.ENEMY' | 'dignity.GREAT_ENEMY' | 'dignity.DEBILITATED' | 'dignity.DEEP_DEBILITATED';
export type DirectionKey = 'direction.EAST' | 'direction.WEST' | 'direction.NORTH' | 'direction.SOUTH' | 'direction.NORTHEAST' | 'direction.NORTHWEST' | 'direction.SOUTHEAST' | 'direction.SOUTHWEST';
export type EraKey = 'era.VIKRAMA' | 'era.SHAKA' | 'era.KALI' | 'era.NEPAL_SAMBAT' | 'era.BUDDHA' | 'era.KOLLAM' | 'era.BENGALI' | 'era.COMMON_ERA' | 'era.BEFORE_COMMON_ERA';
export type GanaKey = 'gana.DEVA' | 'gana.MANUSHYA' | 'gana.RAKSHASA';
export type GrahaKey = 'graha.SUN' | 'graha.MOON' | 'graha.MARS' | 'graha.MERCURY' | 'graha.JUPITER' | 'graha.VENUS' | 'graha.SATURN' | 'graha.RAHU' | 'graha.KETU';
export type HouseSystemKey = 'house_system.WHOLE_SIGN' | 'house_system.PLACIDUS' | 'house_system.KOCH' | 'house_system.REGIOMONTANUS' | 'house_system.CAMPANUS' | 'house_system.EQUAL' | 'house_system.MERIDIAN' | 'house_system.ALCABITIUS' | 'house_system.PORPHYRY' | 'house_system.TOPOCENTRIC' | 'house_system.MORINUS' | 'house_system.SRIPATI' | 'house_system.EQUAL_MC' | 'house_system.EQUAL_ARIES' | 'house_system.VEHLOW' | 'house_system.CARTER' | 'house_system.HORIZON' | 'house_system.SUNSHINE' | 'house_system.PULLEN_SD' | 'house_system.PULLEN_SR' | 'house_system.KRUSINSKI' | 'house_system.APC';
export type KaalaKey = 'kaala.RAHU_KAALA' | 'kaala.YAMAGHANDA' | 'kaala.GULIKA_KAALA';
export type KaranaKey = 'karana.BAVA' | 'karana.BALAVA' | 'karana.KAULAVA' | 'karana.TAITILA' | 'karana.GARIJA' | 'karana.VANIJA' | 'karana.VISHTI' | 'karana.SHAKUNI' | 'karana.CHATUSHPADA' | 'karana.NAGA' | 'karana.KIMSTUGHNA';
export type MasaKey = 'masa.CHAITRA' | 'masa.VAISHAKHA' | 'masa.JYESHTHA' | 'masa.ASHADHA' | 'masa.SHRAVANA' | 'masa.BHADRAPADA' | 'masa.ASHWINA' | 'masa.KARTIKA' | 'masa.MARGASHIRSHA' | 'masa.PAUSHA' | 'masa.MAGHA' | 'masa.PHALGUNA';
export type MuhurtaYogaKey = 'muhurta_yoga.AMRIT_SIDDHI' | 'muhurta_yoga.SARVARTHA_SIDDHI' | 'muhurta_yoga.SIDDHA' | 'muhurta_yoga.DWIPUSHKAR' | 'muhurta_yoga.TRIPUSHKAR';
export type NadiKey = 'nadi.AADI' | 'nadi.MADHYA' | 'nadi.ANTYA';
export type NakshatraKey = 'nakshatra.ASHWINI' | 'nakshatra.BHARANI' | 'nakshatra.KRITTIKA' | 'nakshatra.ROHINI' | 'nakshatra.MRIGASHIRA' | 'nakshatra.ARDRA' | 'nakshatra.PUNARVASU' | 'nakshatra.PUSHYA' | 'nakshatra.ASHLESHA' | 'nakshatra.MAGHA' | 'nakshatra.PURVA_PHALGUNI' | 'nakshatra.UTTARA_PHALGUNI' | 'nakshatra.HASTA' | 'nakshatra.CHITRA' | 'nakshatra.SWATI' | 'nakshatra.VISHAKHA' | 'nakshatra.ANURADHA' | 'nakshatra.JYESHTHA' | 'nakshatra.MULA' | 'nakshatra.PURVA_ASHADHA' | 'nakshatra.UTTARA_ASHADHA' | 'nakshatra.SHRAVANA' | 'nakshatra.DHANISHTHA' | 'nakshatra.SHATABHISHA' | 'nakshatra.PURVA_BHADRAPADA' | 'nakshatra.UTTARA_BHADRAPADA' | 'nakshatra.REVATI';
export type PakshaKey = 'paksha.SHUKLA' | 'paksha.KRISHNA';
export type PanchakaKey = 'panchaka.MRITYU' | 'panchaka.AGNI' | 'panchaka.RAJA' | 'panchaka.CHORA' | 'panchaka.ROGA';
export type PointKey = 'point.LAGNA' | 'point.DHUMA' | 'point.VYATIPATA' | 'point.PARIVESHA' | 'point.INDRACHAPA' | 'point.UPAKETU' | 'point.GULIKA' | 'point.MANDI';
export type RashiKey = 'rashi.ARIES' | 'rashi.TAURUS' | 'rashi.GEMINI' | 'rashi.CANCER' | 'rashi.LEO' | 'rashi.VIRGO' | 'rashi.LIBRA' | 'rashi.SCORPIO' | 'rashi.SAGITTARIUS' | 'rashi.CAPRICORN' | 'rashi.AQUARIUS' | 'rashi.PISCES';
export type RelationshipKey = 'relationship.GREAT_FRIEND' | 'relationship.FRIEND' | 'relationship.NEUTRAL' | 'relationship.ENEMY' | 'relationship.GREAT_ENEMY';
export type SamvatsaraKey = 'samvatsara.PRABHAVA' | 'samvatsara.VIBHAVA' | 'samvatsara.SHUKLA' | 'samvatsara.PRAMODOOTA' | 'samvatsara.PRAJOTPATTI' | 'samvatsara.ANGIRAS' | 'samvatsara.SHRIMUKHA' | 'samvatsara.BHAVA' | 'samvatsara.YUVA' | 'samvatsara.DHAATA' | 'samvatsara.ISHVARA' | 'samvatsara.BAHUDHANYA' | 'samvatsara.PRAMATHI' | 'samvatsara.VIKRAMA' | 'samvatsara.VRISHA' | 'samvatsara.CHITRABHANU' | 'samvatsara.SUBHANU' | 'samvatsara.TAARANA' | 'samvatsara.PAARTHIVA' | 'samvatsara.VYAYA' | 'samvatsara.SARVAJIT' | 'samvatsara.SARVADHARI' | 'samvatsara.VIRODHI' | 'samvatsara.VIKRITA' | 'samvatsara.KHARA' | 'samvatsara.NANDANA' | 'samvatsara.VIJAYA' | 'samvatsara.JAYA' | 'samvatsara.MANMATHA' | 'samvatsara.DURMUKHI' | 'samvatsara.HEVILAMBI' | 'samvatsara.VILAMBI' | 'samvatsara.VIKARI' | 'samvatsara.SHARVARI' | 'samvatsara.PLAVA' | 'samvatsara.SHUBHAKRIT' | 'samvatsara.SHOBHAKRIT' | 'samvatsara.KRODHI' | 'samvatsara.VISHVAVASU' | 'samvatsara.PARABHAVA' | 'samvatsara.PLAVANGA' | 'samvatsara.KILAKA' | 'samvatsara.SAUMYA' | 'samvatsara.SADHARANA' | 'samvatsara.VIRODHIKRIT' | 'samvatsara.PARIDHAAVI' | 'samvatsara.PRAMADICHA' | 'samvatsara.ANANDA' | 'samvatsara.RAKSHASA' | 'samvatsara.NALA' | 'samvatsara.PINGALA' | 'samvatsara.KALAYUKTA' | 'samvatsara.SIDDHARTHI' | 'samvatsara.RAUDRA' | 'samvatsara.DURMATI' | 'samvatsara.DUNDUBHI' | 'samvatsara.RUDHIRODGARI' | 'samvatsara.RAKTAKSHI' | 'samvatsara.KRODHANA' | 'samvatsara.AKSHAYA';
export type StateKey = 'state.RETROGRADE' | 'state.STATIONARY' | 'state.COMBUST' | 'state.PLANETARY_WAR' | 'state.GANDANTA' | 'state.SANDHI' | 'state.VARGOTTAMA' | 'state.PUSHKARA_NAVAMSA' | 'state.PUSHKARA_BHAGA' | 'state.MRITYU_BHAGA' | 'state.MARANA_KARAKA_STHANA' | 'state.ECLIPSED';
export type TatwaKey = 'tatwa.AGNI' | 'tatwa.PRITHVI' | 'tatwa.VAYU' | 'tatwa.JALA' | 'tatwa.AKASHA';
export type TithiKey = 'tithi.SHUKLA_PRATIPADA' | 'tithi.SHUKLA_DVITIYA' | 'tithi.SHUKLA_TRITIYA' | 'tithi.SHUKLA_CHATURTHI' | 'tithi.SHUKLA_PANCHAMI' | 'tithi.SHUKLA_SHASHTHI' | 'tithi.SHUKLA_SAPTAMI' | 'tithi.SHUKLA_ASHTAMI' | 'tithi.SHUKLA_NAVAMI' | 'tithi.SHUKLA_DASHAMI' | 'tithi.SHUKLA_EKADASHI' | 'tithi.SHUKLA_DWADASHI' | 'tithi.SHUKLA_TRAYODASHI' | 'tithi.SHUKLA_CHATURDASHI' | 'tithi.PURNIMA' | 'tithi.KRISHNA_PRATIPADA' | 'tithi.KRISHNA_DVITIYA' | 'tithi.KRISHNA_TRITIYA' | 'tithi.KRISHNA_CHATURTHI' | 'tithi.KRISHNA_PANCHAMI' | 'tithi.KRISHNA_SHASHTHI' | 'tithi.KRISHNA_SAPTAMI' | 'tithi.KRISHNA_ASHTAMI' | 'tithi.KRISHNA_NAVAMI' | 'tithi.KRISHNA_DASHAMI' | 'tithi.KRISHNA_EKADASHI' | 'tithi.KRISHNA_DWADASHI' | 'tithi.KRISHNA_TRAYODASHI' | 'tithi.KRISHNA_CHATURDASHI' | 'tithi.AMAVASYA';
export type VaraKey = 'vara.RAVIVARA' | 'vara.SOMAVARA' | 'vara.MANGALAVARA' | 'vara.BUDHAVARA' | 'vara.GURUVARA' | 'vara.SHUKRAVARA' | 'vara.SHANIVARA';
export type VarnaKey = 'varna.BRAHMIN' | 'varna.KSHATRIYA' | 'varna.VAISHYA' | 'varna.SHUDRA' | 'varna.MLECCHA';
export type YogaKey = 'yoga.VISHKAMBHA' | 'yoga.PRITI' | 'yoga.AYUSHMAN' | 'yoga.SAUBHAGYA' | 'yoga.SHOBHANA' | 'yoga.ATIGANDA' | 'yoga.SUKARMA' | 'yoga.DHRITI' | 'yoga.SHOOLA' | 'yoga.GANDA' | 'yoga.VRIDDHI' | 'yoga.DHRUVA' | 'yoga.VYAGHATA' | 'yoga.HARSHANA' | 'yoga.VAJRA' | 'yoga.SIDDHI' | 'yoga.VYATIPATA' | 'yoga.VARIYANA' | 'yoga.PARIGHA' | 'yoga.SHIVA' | 'yoga.SIDDHA' | 'yoga.SADHYA' | 'yoga.SHUBHA' | 'yoga.SHUKLA' | 'yoga.BRAHMA' | 'yoga.INDRA' | 'yoga.VAIDHRITI';
export type YoniKey = 'yoni.HORSE' | 'yoni.ELEPHANT' | 'yoni.GOAT' | 'yoni.SERPENT' | 'yoni.DOG' | 'yoni.CAT' | 'yoni.RAT' | 'yoni.COW' | 'yoni.BUFFALO' | 'yoni.TIGER' | 'yoni.DEER' | 'yoni.MONKEY' | 'yoni.MONGOOSE' | 'yoni.LION';
export type EntityKey = AvasthaBaladiKey | AvasthaDeeptadiKey | AvasthaJagradadiKey | AvasthaLajjitadiKey | AyanaKey | AyanamshaKey | CharaKarakaKey | ChoghadiyaKey | DashaSystemKey | DeityKey | DignityKey | DirectionKey | EraKey | GanaKey | GrahaKey | HouseSystemKey | KaalaKey | KaranaKey | MasaKey | MuhurtaYogaKey | NadiKey | NakshatraKey | PakshaKey | PanchakaKey | PointKey | RashiKey | RelationshipKey | SamvatsaraKey | StateKey | TatwaKey | TithiKey | VaraKey | VarnaKey | YogaKey | YoniKey;
export type MessageKey = 'sdk.aspect.cast' | 'sdk.aspect.mutual' | 'sdk.calendar.weekdayName' | 'sdk.calendar.weekdayShort' | 'sdk.calendar.GREGORIAN.monthName' | 'sdk.calendar.GREGORIAN.monthShort' | 'sdk.calendar.GREGORIAN.date.numeric' | 'sdk.calendar.GREGORIAN.date.long' | 'sdk.calendar.GREGORIAN.date.full' | 'sdk.calendar.JULIAN.monthName' | 'sdk.calendar.JULIAN.monthShort' | 'sdk.calendar.JULIAN.date.numeric' | 'sdk.calendar.JULIAN.date.long' | 'sdk.calendar.JULIAN.date.full' | 'sdk.calendar.MIXED.monthName' | 'sdk.calendar.MIXED.monthShort' | 'sdk.calendar.MIXED.date.numeric' | 'sdk.calendar.MIXED.date.long' | 'sdk.calendar.MIXED.date.full' | 'sdk.calendar.ISO_WEEK.date.numeric' | 'sdk.calendar.ISO_WEEK.date.long' | 'sdk.calendar.ISO_WEEK.date.full' | 'sdk.calendar.BIKRAM_SAMBAT.monthName' | 'sdk.calendar.BIKRAM_SAMBAT.monthShort' | 'sdk.calendar.BIKRAM_SAMBAT.date.numeric' | 'sdk.calendar.BIKRAM_SAMBAT.date.long' | 'sdk.calendar.BIKRAM_SAMBAT.date.full' | 'sdk.calendar.time.numeric' | 'sdk.calendar.time.long' | 'sdk.calendar.time.numeric12' | 'sdk.calendar.time.long12' | 'sdk.calendar.datetime.join' | 'sdk.calendar.datetime.inZone' | 'sdk.calendar.ghati.numeric' | 'sdk.calendar.ghati.long' | 'sdk.calendar.duration.day' | 'sdk.calendar.duration.hour' | 'sdk.calendar.duration.minute' | 'sdk.calendar.duration.second' | 'sdk.calendar.dayPeriod.am' | 'sdk.calendar.dayPeriod.pm' | 'sdk.calendar.dayPeriod.morning' | 'sdk.calendar.dayPeriod.afternoon' | 'sdk.calendar.dayPeriod.evening' | 'sdk.calendar.dayPeriod.night' | 'sdk.condition.dignity' | 'sdk.condition.navamsha' | 'sdk.condition.vargottama' | 'sdk.condition.retrograde' | 'sdk.condition.combust' | 'sdk.condition.friendship' | 'sdk.condition.age' | 'sdk.condition.wakefulness' | 'sdk.condition.brightness' | 'sdk.condition.lajjitadi' | 'sdk.condition.undecided' | 'sdk.karaka.ofSeven' | 'sdk.karaka.ofEight' | 'sdk.phala.grahaInBhava' | 'sdk.phala.lagnaRashi' | 'sdk.phala.tithi' | 'sdk.phala.vara' | 'sdk.phala.nakshatra' | 'sdk.phala.yoga' | 'sdk.phala.namakarana' | 'sdk.phala.gana' | 'sdk.phala.nadi' | 'sdk.phala.yoni' | 'sdk.phala.varna' | 'sdk.phala.tatwa' | 'sdk.phala.dashaLord' | 'sdk.phala.dashaActivation' | 'sdk.phala.avasthaBaladi' | 'sdk.phala.avasthaJagradadi' | 'sdk.phala.avasthaDeeptadi' | 'sdk.phala.avasthaLajjitadi' | 'sdk.phala.dignity' | 'sdk.phala.state' | 'sdk.reading.effect' | 'sdk.reading.says' | 'sdk.reading.timing' | 'sdk.reading.lifeSpan' | 'sdk.reading.lifeClass' | 'sdk.reading.participants' | 'sdk.reading.status' | 'sdk.reading.severity' | 'sdk.reason.appName' | 'sdk.reason.welcome' | 'sdk.reason.grahaInBhava' | 'sdk.reason.bhavaInRashi' | 'sdk.reason.chalitShift' | 'sdk.reason.grahaInRashi' | 'sdk.reason.pointInRashi' | 'sdk.reason.grahaAt' | 'sdk.reason.pointAt' | 'sdk.reason.exactLongitude' | 'sdk.reason.conjunction' | 'sdk.reason.rashiNature' | 'sdk.reason.greeting' | 'sdk.reason.lordship' | 'sdk.reason.occupants' | 'sdk.reason.strength.score' | 'sdk.reason.strength.meets' | 'sdk.reason.strength.rank' | 'sdk.reason.dasha.phase' | 'sdk.reason.dasha.place' | 'sdk.reason.dasha.points' | 'sdk.reason.dasha.favour' | 'sdk.reason.panchanga.tithi' | 'sdk.reason.panchanga.vara' | 'sdk.reason.panchanga.nakshatra' | 'sdk.reason.panchanga.pada' | 'sdk.reason.panchanga.yoga' | 'sdk.reason.panchanga.karana' | 'sdk.reason.panchanga.byDay' | 'sdk.reason.bhavaBala' | 'sdk.reason.vimshopaka' | 'sdk.reason.ashtakavarga' | 'sdk.reason.sarvashtakavarga';

export interface DateValue { readonly calendar: string; readonly year: number; readonly month: number; readonly day: number; }
export interface TimeValue { readonly hour: number; readonly minute: number; readonly second: number; }
export interface DateTimeValue { readonly date: DateValue; readonly time: TimeValue; }
export interface GhatiValue { readonly ghati: number; readonly pala: number; readonly vipala: number; }
/** An instant, the SDK's own: a Julian day in UTC, read in the zone a message's `timeZone` names. */
export type InstantValue = number;

export interface EntityForms {
  readonly iast: string;
  readonly name: string;
  readonly prose: string;
  readonly short?: string;
  readonly glyph?: string;
  readonly gender?: Gender;
  /**
   * Every form the locale carries, including any a loaded pack brought:
   * a record's forms are an open set, and the fields above are only the
   * ones `i18n/` guarantees.
   */
  readonly forms: Readonly<Record<string, string>>;
}

export interface Renderer {
  render(key: MessageKey, params?: Readonly<Record<string, unknown>>): string;
  entity(key: EntityKey): EntityForms;
}

export interface Messages {
  readonly sdk: {
    readonly aspect: {
      readonly cast: (p: { from: GrahaKey; strength: string; to: GrahaKey }) => string;
      readonly mutual: (p: { first: GrahaKey; second: GrahaKey }) => string;
    };
    readonly calendar: {
      readonly bikramSambat: {
        readonly date: {
          readonly full: (p: { day: number; monthName: string; weekdayName: string; year: number }) => string;
          readonly long: (p: { day: number; monthName: string; year: number }) => string;
          readonly numeric: (p: { day: number; month: number; year: number }) => string;
        };
        readonly monthName: (p: { month: number }) => string;
        readonly monthShort: (p: { month: number }) => string;
      };
      readonly gregorian: {
        readonly date: {
          readonly full: (p: { day: number; monthName: string; weekdayName: string; year: number }) => string;
          readonly long: (p: { day: number; monthName: string; year: number }) => string;
          readonly numeric: (p: { day: number; month: number; year: number }) => string;
        };
        readonly monthName: (p: { month: number }) => string;
        readonly monthShort: (p: { month: number }) => string;
      };
      readonly isoWeek: {
        readonly date: {
          readonly full: () => string;
          readonly long: (p: { day: number; month: number; year: number }) => string;
          readonly numeric: (p: { day: number; month: number; year: number }) => string;
        };
      };
      readonly julian: {
        readonly date: {
          readonly full: () => string;
          readonly long: () => string;
          readonly numeric: () => string;
        };
        readonly monthName: () => string;
        readonly monthShort: () => string;
      };
      readonly mixed: {
        readonly date: {
          readonly full: () => string;
          readonly long: () => string;
          readonly numeric: () => string;
        };
        readonly monthName: () => string;
        readonly monthShort: () => string;
      };
      readonly datetime: {
        readonly inZone: (p: { at: InstantValue; zone: string }) => string;
        readonly join: (p: { date: string; time: string }) => string;
      };
      readonly dayPeriod: {
        readonly afternoon: () => string;
        readonly am: () => string;
        readonly evening: () => string;
        readonly morning: () => string;
        readonly night: () => string;
        readonly pm: () => string;
      };
      readonly duration: {
        readonly day: (p: { n: number }) => string;
        readonly hour: (p: { n: number }) => string;
        readonly minute: (p: { n: number }) => string;
        readonly second: (p: { n: number }) => string;
      };
      readonly ghati: {
        readonly long: (p: { ghati: number; pala: number }) => string;
        readonly numeric: (p: { ghati: number; pala: number }) => string;
      };
      readonly time: {
        readonly long: (p: { hour: number; minute: number; second: number }) => string;
        readonly long12: (p: { hour12: number; meridiem: string; minute: number; second: number }) => string;
        readonly numeric: (p: { hour: number; minute: number }) => string;
        readonly numeric12: (p: { hour12: number; meridiem: string; minute: number }) => string;
      };
      readonly weekdayName: (p: { weekday: number }) => string;
      readonly weekdayShort: (p: { weekday: number }) => string;
    };
    readonly condition: {
      readonly age: (p: { age: AvasthaBaladiKey; graha: GrahaKey }) => string;
      readonly brightness: (p: { deeptadi: AvasthaDeeptadiKey; graha: GrahaKey }) => string;
      readonly combust: (p: { graha: GrahaKey }) => string;
      readonly dignity: (p: { dignity: DignityKey; graha: GrahaKey }) => string;
      readonly friendship: (p: { compound: RelationshipKey; dispositor: GrahaKey; graha: GrahaKey; natural: RelationshipKey; temporary: RelationshipKey }) => string;
      readonly lajjitadi: (p: { graha: GrahaKey; states: readonly (string | EntityKey)[] }) => string;
      readonly navamsha: (p: { graha: GrahaKey; rashi: RashiKey }) => string;
      readonly retrograde: (p: { graha: GrahaKey }) => string;
      readonly undecided: (p: { count: number; graha: GrahaKey; states: readonly (string | EntityKey)[] }) => string;
      readonly vargottama: (p: { graha: GrahaKey }) => string;
      readonly wakefulness: (p: { graha: GrahaKey; wakefulness: AvasthaJagradadiKey }) => string;
    };
    readonly entity: {
      readonly avasthaBaladi: {
        readonly bala: () => EntityForms;
        readonly kumara: () => EntityForms;
        readonly mrita: () => EntityForms;
        readonly vriddha: () => EntityForms;
        readonly yuva: () => EntityForms;
      };
      readonly avasthaDeeptadi: {
        readonly deepta: () => EntityForms;
        readonly dina: () => EntityForms;
        readonly dukhi: () => EntityForms;
        readonly khala: () => EntityForms;
        readonly kopa: () => EntityForms;
        readonly mudita: () => EntityForms;
        readonly shanta: () => EntityForms;
        readonly swastha: () => EntityForms;
        readonly vikala: () => EntityForms;
      };
      readonly avasthaJagradadi: {
        readonly jagrat: () => EntityForms;
        readonly sushupti: () => EntityForms;
        readonly swapna: () => EntityForms;
      };
      readonly avasthaLajjitadi: {
        readonly garvita: () => EntityForms;
        readonly kshobhita: () => EntityForms;
        readonly kshudha: () => EntityForms;
        readonly lajjita: () => EntityForms;
        readonly mudita: () => EntityForms;
        readonly trishita: () => EntityForms;
      };
      readonly ayana: {
        readonly dakshinayana: () => EntityForms;
        readonly uttarayana: () => EntityForms;
      };
      readonly ayanamsha: {
        readonly aldebaran15Tau: () => EntityForms;
        readonly aryabhata: () => EntityForms;
        readonly aryabhata522: () => EntityForms;
        readonly aryabhataMsun: () => EntityForms;
        readonly b1950: () => EntityForms;
        readonly babylBritton: () => EntityForms;
        readonly babylEtpsc: () => EntityForms;
        readonly babylHuber: () => EntityForms;
        readonly babylKugler1: () => EntityForms;
        readonly babylKugler2: () => EntityForms;
        readonly babylKugler3: () => EntityForms;
        readonly deluce: () => EntityForms;
        readonly djwhalKhul: () => EntityForms;
        readonly faganBradley: () => EntityForms;
        readonly galalignMardyks: () => EntityForms;
        readonly galcent0Sag: () => EntityForms;
        readonly galcentCochrane: () => EntityForms;
        readonly galcentMulaWilhelm: () => EntityForms;
        readonly galcentRgilbrand: () => EntityForms;
        readonly galequFiorenza: () => EntityForms;
        readonly galequIau1958: () => EntityForms;
        readonly galequMula: () => EntityForms;
        readonly galequTrue: () => EntityForms;
        readonly hipparchos: () => EntityForms;
        readonly j1900: () => EntityForms;
        readonly j2000: () => EntityForms;
        readonly jnBhasin: () => EntityForms;
        readonly krishnamurti: () => EntityForms;
        readonly krishnamurtiVp291: () => EntityForms;
        readonly lahiri: () => EntityForms;
        readonly lahiri1940: () => EntityForms;
        readonly lahiriIcrc: () => EntityForms;
        readonly lahiriVp285: () => EntityForms;
        readonly raman: () => EntityForms;
        readonly sassanian: () => EntityForms;
        readonly ssCitra: () => EntityForms;
        readonly ssRevati: () => EntityForms;
        readonly suryasiddhanta: () => EntityForms;
        readonly suryasiddhantaMsun: () => EntityForms;
        readonly trueChitra: () => EntityForms;
        readonly trueMula: () => EntityForms;
        readonly truePushya: () => EntityForms;
        readonly trueRevati: () => EntityForms;
        readonly trueSheoran: () => EntityForms;
        readonly ushashashi: () => EntityForms;
        readonly valensMoon: () => EntityForms;
        readonly yukteshwar: () => EntityForms;
      };
      readonly charaKaraka: {
        readonly amatyakaraka: () => EntityForms;
        readonly atmakaraka: () => EntityForms;
        readonly bhratrikaraka: () => EntityForms;
        readonly darakaraka: () => EntityForms;
        readonly gnatikaraka: () => EntityForms;
        readonly matrikaraka: () => EntityForms;
        readonly pitrikaraka: () => EntityForms;
        readonly putrakaraka: () => EntityForms;
      };
      readonly choghadiya: {
        readonly amrit: () => EntityForms;
        readonly char: () => EntityForms;
        readonly kaal: () => EntityForms;
        readonly laabh: () => EntityForms;
        readonly rog: () => EntityForms;
        readonly shubha: () => EntityForms;
        readonly udveg: () => EntityForms;
      };
      readonly dashaSystem: {
        readonly ashtottari: () => EntityForms;
        readonly chara: () => EntityForms;
        readonly chaturashitiSama: () => EntityForms;
        readonly drig: () => EntityForms;
        readonly dwadashottari: () => EntityForms;
        readonly dwisaptatiSama: () => EntityForms;
        readonly kalachakra: () => EntityForms;
        readonly mandooka: () => EntityForms;
        readonly narayana: () => EntityForms;
        readonly niryanaShoola: () => EntityForms;
        readonly padanadhamsa: () => EntityForms;
        readonly panchottari: () => EntityForms;
        readonly shatabdika: () => EntityForms;
        readonly shoola: () => EntityForms;
        readonly tribhagi: () => EntityForms;
        readonly trikona: () => EntityForms;
        readonly vimshottari: () => EntityForms;
        readonly yogini: () => EntityForms;
      };
      readonly deity: {
        readonly aditi: () => EntityForms;
        readonly agni: () => EntityForms;
        readonly ahirbudhnya: () => EntityForms;
        readonly ajaEkapada: () => EntityForms;
        readonly apas: () => EntityForms;
        readonly aryaman: () => EntityForms;
        readonly ashwiniKumara: () => EntityForms;
        readonly bhaga: () => EntityForms;
        readonly brahma: () => EntityForms;
        readonly brihaspati: () => EntityForms;
        readonly indra: () => EntityForms;
        readonly indragni: () => EntityForms;
        readonly mitra: () => EntityForms;
        readonly naga: () => EntityForms;
        readonly nirriti: () => EntityForms;
        readonly pitru: () => EntityForms;
        readonly pushan: () => EntityForms;
        readonly rudra: () => EntityForms;
        readonly savitr: () => EntityForms;
        readonly soma: () => EntityForms;
        readonly varuna: () => EntityForms;
        readonly vasu: () => EntityForms;
        readonly vayu: () => EntityForms;
        readonly vishnu: () => EntityForms;
        readonly vishvadeva: () => EntityForms;
        readonly vishvakarma: () => EntityForms;
        readonly yama: () => EntityForms;
      };
      readonly dignity: {
        readonly debilitated: () => EntityForms;
        readonly deepDebilitated: () => EntityForms;
        readonly deepExalted: () => EntityForms;
        readonly enemy: () => EntityForms;
        readonly exalted: () => EntityForms;
        readonly friend: () => EntityForms;
        readonly greatEnemy: () => EntityForms;
        readonly greatFriend: () => EntityForms;
        readonly mooltrikona: () => EntityForms;
        readonly neutral: () => EntityForms;
        readonly ownSign: () => EntityForms;
      };
      readonly direction: {
        readonly east: () => EntityForms;
        readonly north: () => EntityForms;
        readonly northeast: () => EntityForms;
        readonly northwest: () => EntityForms;
        readonly south: () => EntityForms;
        readonly southeast: () => EntityForms;
        readonly southwest: () => EntityForms;
        readonly west: () => EntityForms;
      };
      readonly era: {
        readonly beforeCommonEra: () => EntityForms;
        readonly bengali: () => EntityForms;
        readonly buddha: () => EntityForms;
        readonly commonEra: () => EntityForms;
        readonly kali: () => EntityForms;
        readonly kollam: () => EntityForms;
        readonly nepalSambat: () => EntityForms;
        readonly shaka: () => EntityForms;
        readonly vikrama: () => EntityForms;
      };
      readonly gana: {
        readonly deva: () => EntityForms;
        readonly manushya: () => EntityForms;
        readonly rakshasa: () => EntityForms;
      };
      readonly graha: {
        readonly jupiter: () => EntityForms;
        readonly ketu: () => EntityForms;
        readonly mars: () => EntityForms;
        readonly mercury: () => EntityForms;
        readonly moon: () => EntityForms;
        readonly rahu: () => EntityForms;
        readonly saturn: () => EntityForms;
        readonly sun: () => EntityForms;
        readonly venus: () => EntityForms;
      };
      readonly houseSystem: {
        readonly alcabitius: () => EntityForms;
        readonly apc: () => EntityForms;
        readonly campanus: () => EntityForms;
        readonly carter: () => EntityForms;
        readonly equal: () => EntityForms;
        readonly equalAries: () => EntityForms;
        readonly equalMc: () => EntityForms;
        readonly horizon: () => EntityForms;
        readonly koch: () => EntityForms;
        readonly krusinski: () => EntityForms;
        readonly meridian: () => EntityForms;
        readonly morinus: () => EntityForms;
        readonly placidus: () => EntityForms;
        readonly porphyry: () => EntityForms;
        readonly pullenSd: () => EntityForms;
        readonly pullenSr: () => EntityForms;
        readonly regiomontanus: () => EntityForms;
        readonly sripati: () => EntityForms;
        readonly sunshine: () => EntityForms;
        readonly topocentric: () => EntityForms;
        readonly vehlow: () => EntityForms;
        readonly wholeSign: () => EntityForms;
      };
      readonly kaala: {
        readonly gulikaKaala: () => EntityForms;
        readonly rahuKaala: () => EntityForms;
        readonly yamaghanda: () => EntityForms;
      };
      readonly karana: {
        readonly balava: () => EntityForms;
        readonly bava: () => EntityForms;
        readonly chatushpada: () => EntityForms;
        readonly garija: () => EntityForms;
        readonly kaulava: () => EntityForms;
        readonly kimstughna: () => EntityForms;
        readonly naga: () => EntityForms;
        readonly shakuni: () => EntityForms;
        readonly taitila: () => EntityForms;
        readonly vanija: () => EntityForms;
        readonly vishti: () => EntityForms;
      };
      readonly masa: {
        readonly ashadha: () => EntityForms;
        readonly ashwina: () => EntityForms;
        readonly bhadrapada: () => EntityForms;
        readonly chaitra: () => EntityForms;
        readonly jyeshtha: () => EntityForms;
        readonly kartika: () => EntityForms;
        readonly magha: () => EntityForms;
        readonly margashirsha: () => EntityForms;
        readonly pausha: () => EntityForms;
        readonly phalguna: () => EntityForms;
        readonly shravana: () => EntityForms;
        readonly vaishakha: () => EntityForms;
      };
      readonly muhurtaYoga: {
        readonly amritSiddhi: () => EntityForms;
        readonly dwipushkar: () => EntityForms;
        readonly sarvarthaSiddhi: () => EntityForms;
        readonly siddha: () => EntityForms;
        readonly tripushkar: () => EntityForms;
      };
      readonly nadi: {
        readonly aadi: () => EntityForms;
        readonly antya: () => EntityForms;
        readonly madhya: () => EntityForms;
      };
      readonly nakshatra: {
        readonly anuradha: () => EntityForms;
        readonly ardra: () => EntityForms;
        readonly ashlesha: () => EntityForms;
        readonly ashwini: () => EntityForms;
        readonly bharani: () => EntityForms;
        readonly chitra: () => EntityForms;
        readonly dhanishtha: () => EntityForms;
        readonly hasta: () => EntityForms;
        readonly jyeshtha: () => EntityForms;
        readonly krittika: () => EntityForms;
        readonly magha: () => EntityForms;
        readonly mrigashira: () => EntityForms;
        readonly mula: () => EntityForms;
        readonly punarvasu: () => EntityForms;
        readonly purvaAshadha: () => EntityForms;
        readonly purvaBhadrapada: () => EntityForms;
        readonly purvaPhalguni: () => EntityForms;
        readonly pushya: () => EntityForms;
        readonly revati: () => EntityForms;
        readonly rohini: () => EntityForms;
        readonly shatabhisha: () => EntityForms;
        readonly shravana: () => EntityForms;
        readonly swati: () => EntityForms;
        readonly uttaraAshadha: () => EntityForms;
        readonly uttaraBhadrapada: () => EntityForms;
        readonly uttaraPhalguni: () => EntityForms;
        readonly vishakha: () => EntityForms;
      };
      readonly paksha: {
        readonly krishna: () => EntityForms;
        readonly shukla: () => EntityForms;
      };
      readonly panchaka: {
        readonly agni: () => EntityForms;
        readonly chora: () => EntityForms;
        readonly mrityu: () => EntityForms;
        readonly raja: () => EntityForms;
        readonly roga: () => EntityForms;
      };
      readonly point: {
        readonly dhuma: () => EntityForms;
        readonly gulika: () => EntityForms;
        readonly indrachapa: () => EntityForms;
        readonly lagna: () => EntityForms;
        readonly mandi: () => EntityForms;
        readonly parivesha: () => EntityForms;
        readonly upaketu: () => EntityForms;
        readonly vyatipata: () => EntityForms;
      };
      readonly rashi: {
        readonly aquarius: () => EntityForms;
        readonly aries: () => EntityForms;
        readonly cancer: () => EntityForms;
        readonly capricorn: () => EntityForms;
        readonly gemini: () => EntityForms;
        readonly leo: () => EntityForms;
        readonly libra: () => EntityForms;
        readonly pisces: () => EntityForms;
        readonly sagittarius: () => EntityForms;
        readonly scorpio: () => EntityForms;
        readonly taurus: () => EntityForms;
        readonly virgo: () => EntityForms;
      };
      readonly relationship: {
        readonly enemy: () => EntityForms;
        readonly friend: () => EntityForms;
        readonly greatEnemy: () => EntityForms;
        readonly greatFriend: () => EntityForms;
        readonly neutral: () => EntityForms;
      };
      readonly samvatsara: {
        readonly akshaya: () => EntityForms;
        readonly ananda: () => EntityForms;
        readonly angiras: () => EntityForms;
        readonly bahudhanya: () => EntityForms;
        readonly bhava: () => EntityForms;
        readonly chitrabhanu: () => EntityForms;
        readonly dhaata: () => EntityForms;
        readonly dundubhi: () => EntityForms;
        readonly durmati: () => EntityForms;
        readonly durmukhi: () => EntityForms;
        readonly hevilambi: () => EntityForms;
        readonly ishvara: () => EntityForms;
        readonly jaya: () => EntityForms;
        readonly kalayukta: () => EntityForms;
        readonly khara: () => EntityForms;
        readonly kilaka: () => EntityForms;
        readonly krodhana: () => EntityForms;
        readonly krodhi: () => EntityForms;
        readonly manmatha: () => EntityForms;
        readonly nala: () => EntityForms;
        readonly nandana: () => EntityForms;
        readonly paarthiva: () => EntityForms;
        readonly parabhava: () => EntityForms;
        readonly paridhaavi: () => EntityForms;
        readonly pingala: () => EntityForms;
        readonly plava: () => EntityForms;
        readonly plavanga: () => EntityForms;
        readonly prabhava: () => EntityForms;
        readonly prajotpatti: () => EntityForms;
        readonly pramadicha: () => EntityForms;
        readonly pramathi: () => EntityForms;
        readonly pramodoota: () => EntityForms;
        readonly rakshasa: () => EntityForms;
        readonly raktakshi: () => EntityForms;
        readonly raudra: () => EntityForms;
        readonly rudhirodgari: () => EntityForms;
        readonly sadharana: () => EntityForms;
        readonly sarvadhari: () => EntityForms;
        readonly sarvajit: () => EntityForms;
        readonly saumya: () => EntityForms;
        readonly sharvari: () => EntityForms;
        readonly shobhakrit: () => EntityForms;
        readonly shrimukha: () => EntityForms;
        readonly shubhakrit: () => EntityForms;
        readonly shukla: () => EntityForms;
        readonly siddharthi: () => EntityForms;
        readonly subhanu: () => EntityForms;
        readonly taarana: () => EntityForms;
        readonly vibhava: () => EntityForms;
        readonly vijaya: () => EntityForms;
        readonly vikari: () => EntityForms;
        readonly vikrama: () => EntityForms;
        readonly vikrita: () => EntityForms;
        readonly vilambi: () => EntityForms;
        readonly virodhi: () => EntityForms;
        readonly virodhikrit: () => EntityForms;
        readonly vishvavasu: () => EntityForms;
        readonly vrisha: () => EntityForms;
        readonly vyaya: () => EntityForms;
        readonly yuva: () => EntityForms;
      };
      readonly tatwa: {
        readonly agni: () => EntityForms;
        readonly akasha: () => EntityForms;
        readonly jala: () => EntityForms;
        readonly prithvi: () => EntityForms;
        readonly vayu: () => EntityForms;
      };
      readonly tithi: {
        readonly amavasya: () => EntityForms;
        readonly krishnaAshtami: () => EntityForms;
        readonly krishnaChaturdashi: () => EntityForms;
        readonly krishnaChaturthi: () => EntityForms;
        readonly krishnaDashami: () => EntityForms;
        readonly krishnaDvitiya: () => EntityForms;
        readonly krishnaDwadashi: () => EntityForms;
        readonly krishnaEkadashi: () => EntityForms;
        readonly krishnaNavami: () => EntityForms;
        readonly krishnaPanchami: () => EntityForms;
        readonly krishnaPratipada: () => EntityForms;
        readonly krishnaSaptami: () => EntityForms;
        readonly krishnaShashthi: () => EntityForms;
        readonly krishnaTrayodashi: () => EntityForms;
        readonly krishnaTritiya: () => EntityForms;
        readonly purnima: () => EntityForms;
        readonly shuklaAshtami: () => EntityForms;
        readonly shuklaChaturdashi: () => EntityForms;
        readonly shuklaChaturthi: () => EntityForms;
        readonly shuklaDashami: () => EntityForms;
        readonly shuklaDvitiya: () => EntityForms;
        readonly shuklaDwadashi: () => EntityForms;
        readonly shuklaEkadashi: () => EntityForms;
        readonly shuklaNavami: () => EntityForms;
        readonly shuklaPanchami: () => EntityForms;
        readonly shuklaPratipada: () => EntityForms;
        readonly shuklaSaptami: () => EntityForms;
        readonly shuklaShashthi: () => EntityForms;
        readonly shuklaTrayodashi: () => EntityForms;
        readonly shuklaTritiya: () => EntityForms;
      };
      readonly vara: {
        readonly budhavara: () => EntityForms;
        readonly guruvara: () => EntityForms;
        readonly mangalavara: () => EntityForms;
        readonly ravivara: () => EntityForms;
        readonly shanivara: () => EntityForms;
        readonly shukravara: () => EntityForms;
        readonly somavara: () => EntityForms;
      };
      readonly varna: {
        readonly brahmin: () => EntityForms;
        readonly kshatriya: () => EntityForms;
        readonly mleccha: () => EntityForms;
        readonly shudra: () => EntityForms;
        readonly vaishya: () => EntityForms;
      };
      readonly yoga: {
        readonly atiganda: () => EntityForms;
        readonly ayushman: () => EntityForms;
        readonly brahma: () => EntityForms;
        readonly dhriti: () => EntityForms;
        readonly dhruva: () => EntityForms;
        readonly ganda: () => EntityForms;
        readonly harshana: () => EntityForms;
        readonly indra: () => EntityForms;
        readonly parigha: () => EntityForms;
        readonly priti: () => EntityForms;
        readonly sadhya: () => EntityForms;
        readonly saubhagya: () => EntityForms;
        readonly shiva: () => EntityForms;
        readonly shobhana: () => EntityForms;
        readonly shoola: () => EntityForms;
        readonly shubha: () => EntityForms;
        readonly shukla: () => EntityForms;
        readonly siddha: () => EntityForms;
        readonly siddhi: () => EntityForms;
        readonly sukarma: () => EntityForms;
        readonly vaidhriti: () => EntityForms;
        readonly vajra: () => EntityForms;
        readonly variyana: () => EntityForms;
        readonly vishkambha: () => EntityForms;
        readonly vriddhi: () => EntityForms;
        readonly vyaghata: () => EntityForms;
        readonly vyatipata: () => EntityForms;
      };
      readonly yoni: {
        readonly buffalo: () => EntityForms;
        readonly cat: () => EntityForms;
        readonly cow: () => EntityForms;
        readonly deer: () => EntityForms;
        readonly dog: () => EntityForms;
        readonly elephant: () => EntityForms;
        readonly goat: () => EntityForms;
        readonly horse: () => EntityForms;
        readonly lion: () => EntityForms;
        readonly mongoose: () => EntityForms;
        readonly monkey: () => EntityForms;
        readonly rat: () => EntityForms;
        readonly serpent: () => EntityForms;
        readonly tiger: () => EntityForms;
      };
    };
    readonly karaka: {
      readonly ofEight: (p: { graha: GrahaKey; karaka: CharaKarakaKey }) => string;
      readonly ofSeven: (p: { graha: GrahaKey; karaka: CharaKarakaKey }) => string;
    };
    readonly phala: {
      readonly avasthaBaladi: (p: { avastha: AvasthaBaladiKey }) => string;
      readonly avasthaDeeptadi: (p: { avastha: AvasthaDeeptadiKey }) => string;
      readonly avasthaJagradadi: (p: { avastha: AvasthaJagradadiKey }) => string;
      readonly avasthaLajjitadi: (p: { avastha: AvasthaLajjitadiKey }) => string;
      readonly dashaActivation: (p: { graha: GrahaKey }) => string;
      readonly dashaLord: (p: { graha: GrahaKey }) => string;
      readonly dignity: (p: { dignity: DignityKey }) => string;
      readonly gana: (p: { gana: GanaKey }) => string;
      readonly grahaInBhava: (p: { bhava: number; graha: GrahaKey; phala: EntityKey }) => string;
      readonly lagnaRashi: (p: { rashi: RashiKey }) => string;
      readonly nadi: (p: { nadi: NadiKey }) => string;
      readonly nakshatra: (p: { nakshatra: NakshatraKey }) => string;
      readonly namakarana: (p: { nakshatra: NakshatraKey }) => string;
      readonly state: (p: { state: StateKey }) => string;
      readonly tatwa: (p: { tatwa: TatwaKey }) => string;
      readonly tithi: (p: { tithi: TithiKey }) => string;
      readonly vara: (p: { vara: VaraKey }) => string;
      readonly varna: (p: { varna: VarnaKey }) => string;
      readonly yoga: (p: { yoga: YogaKey }) => string;
      readonly yoni: (p: { yoni: YoniKey }) => string;
    };
    readonly reading: {
      readonly effect: (p: { rule: string; text: string }) => string;
      readonly lifeClass: (p: { class: string; rule: string }) => string;
      readonly lifeSpan: (p: { count: number; rule: string; unit: string }) => string;
      readonly participants: (p: { count: number; grahas: readonly (string | EntityKey)[]; rule: string }) => string;
      readonly says: (p: { reading: EntityKey; rule: string }) => string;
      readonly severity: (p: { rule: string; severity: number }) => string;
      readonly status: (p: { rule: string; status: string }) => string;
      readonly timing: (p: { reading: EntityKey; rule: string }) => string;
    };
    readonly reason: {
      readonly appName: () => string;
      readonly ashtakavarga: (p: { bindus: number; graha: GrahaKey; rashi: RashiKey }) => string;
      readonly bhavaBala: (p: { bhava: number; virupas: number }) => string;
      readonly bhavaInRashi: (p: { bhava: number; rashi: RashiKey }) => string;
      readonly chalitShift: (p: { bhava: number; chalit: number; graha: GrahaKey }) => string;
      readonly conjunction: (p: { count: number; graha: GrahaKey }) => string;
      readonly dasha: {
        readonly favour: (p: { favour: string; graha: GrahaKey }) => string;
        readonly phase: (p: { graha: GrahaKey; phase: string }) => string;
        readonly place: (p: { graha: GrahaKey; nature: string }) => string;
        readonly points: (p: { asubhanka: number; graha: GrahaKey; subhanka: number }) => string;
      };
      readonly exactLongitude: (p: { longitude: number }) => string;
      readonly grahaAt: (p: { graha: GrahaKey; longitude: number }) => string;
      readonly grahaInBhava: (p: { bhava: number; graha: GrahaKey }) => string;
      readonly grahaInRashi: (p: { graha: GrahaKey; rashi: RashiKey }) => string;
      readonly greeting: (p: { gender: Gender; name: string }) => string;
      readonly lordship: (p: { bhava: number; graha: GrahaKey }) => string;
      readonly occupants: (p: { grahas: readonly (string | EntityKey)[]; rashi: RashiKey }) => string;
      readonly panchanga: {
        readonly byDay: (p: { byDay: string }) => string;
        readonly karana: (p: { karana: KaranaKey }) => string;
        readonly nakshatra: (p: { nakshatra: NakshatraKey }) => string;
        readonly pada: (p: { nakshatra: NakshatraKey; pada: number }) => string;
        readonly tithi: (p: { paksha: PakshaKey; tithi: TithiKey }) => string;
        readonly vara: (p: { vara: VaraKey }) => string;
        readonly yoga: (p: { yoga: YogaKey }) => string;
      };
      readonly pointAt: (p: { longitude: number; point: PointKey }) => string;
      readonly pointInRashi: (p: { point: PointKey; rashi: RashiKey }) => string;
      readonly rashiNature: (p: { rashi: RashiKey }) => string;
      readonly sarvashtakavarga: (p: { bindus: number; rashi: RashiKey }) => string;
      readonly strength: {
        readonly meets: (p: { graha: GrahaKey; reaches: string; required: number }) => string;
        readonly rank: (p: { rank: number }) => string;
        readonly score: (p: { graha: GrahaKey; score: number }) => string;
      };
      readonly vimshopaka: (p: { graha: GrahaKey; scheme: string; score: number }) => string;
      readonly welcome: () => string;
    };
  };
}

/** The typed accessors over a renderer. */
export declare function messages(r: Renderer): Messages;

/** An entity's forms, as `ts_intl_entity` hands them out. */
export declare function entityForms(json: string | Readonly<Record<string, unknown>>): EntityForms;
